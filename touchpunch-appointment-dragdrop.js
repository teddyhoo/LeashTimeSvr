/* touchpunch-appointment-dragdrop.js */
...
function refreshAppointmentReassignmentTables() {
  ...
  //alert(wagButton);
	...
	$('.PageBackgroundGradient,.Main').addClass('ui-droppable');
	$('.PageBackgroundGradient,.Main').droppable({
		 drop:function(event, ui) {
			 alert("Please drop this visit on a sitter.");
			 refreshAppointmentReassignmentTables();				
		}});
...
function ajaxGetAppts(url, target) {
  ...
  xh.onreadystatechange=function() { if(xh.readyState==4) {
	...
	$('.ui-draggable').draggable({helper: "original"});
	}
  xh.send(null);
}
