...
$slideTest = false; // mattOnlyTEST(); -- rejected early on.  Also: causes cross-origin Javascript blocking.
...
.multiunpaidbillables {background: pink !important;}
</style>
<?
...
//if(mattOnlyTEST()) print_r($prepayments[37]);
	completePrepaymentsArray($prepayments, $repeatCustomers, $recurring, $section);
...
//if(mattOnlyTEST()) echo "{$pp['fname']} {$pp['lname']} {$pp['prepayment']} &lt;= {$pp['creditvalue']}<hr>";
			$shouldBeHidden = TRUE ? ($pp['netdueraw'] < .01 /*<= 0*/) : ($pp['prepayment'] <= $pp['creditvalue']);
...
//if($goMinilog) print_r($prepayments);
	...
	$sumNetDueFloat = $sumNetDue ? $sumNetDue : '';
	...
	foreach($prepayments as $i => $pp) {
		$nextRowClass = $rowClass = $rowClass == 'clientrow futuretask' ? 'clientrow futuretaskEVEN' : 'clientrow futuretask';
		if($pp['multiBillables']) $nextRowClass = "$nextRowClass multiunpaidbillables";
		$rowClasses[] = $nextRowClass;
	}
	...
	<tr><td>$paymentsDue</td><td id='sumNetDue_$section' due='$sumNetDueFloat'>$sumNetDue</td><td id='sumCredits_$section'>$sumCredits</td></tr></table>";
	echo "</span>";
	echo "</form>";
	...
	if($prepayments) {
		if(mattOnlyTEST()) { // for Pawsitively Pooches, to generate a summary of invoices to be emailed to a building complex
			echoButton('', 'Summary', "summarizeSelectedInvoices(\"$section\", this)"); echo "<img src='art/spacer.gif' width=5>";
		}
...
//$templates = false;			
		...
		if(dbTEST('leashtimecustomers')) {
			echo ' ';
			echoButton('', 'Toggle Lockouts', 'toggleLockouts()');
			echo " <span id='totalsWithLockouts'></span>";
...
function lockedOut($clientid) {
	$bizid = fetchRow0Col0("SELECT garagegatecode FROM tblclient WHERE clientid = $clientid LIMIT 1", 1);
	if(!$bizid) return;
	global $dbhost, $db, $dbuser, $dbpass;
	list($dbhost1, $db1, $dbuser1, $dbpass1) = array($dbhost, $db, $dbuser, $dbpass);
	include "common/init_db_common.php";
	$lockedout = fetchRow0Col0("SELECT lockout FROM tblpetbiz WHERE bizid = $bizid LIMIT 1", 1);
			...
			if(dbTEST('leashtimecustomers')) {
				$unpaidbillables = fetchRow0Col0(
					"SELECT COUNT(*) 
						FROM tblbillable
						WHERE superseded = 0
							AND clientptr = {$pp['clientid']}
							AND paid < charge", 1);
				$pp['multiBillables'] = $unpaidbillables > 1;
				$unpaidbillables = $unpaidbillables > 1 ? " Unpaid billables: $unpaidbillables" : '';
				if($lockoutdate = lockedOut($pp['clientid'])) 
					$pp['clientname'] .= 
						"<br><span class='lockout warning' title='locked out'><img src='art/lockout-red.gif'> "
						.date('m/d/Y', strtotime($lockoutdate))
						.".$unpaidbillables</span>";
				else if($unpaidbillables) $pp['clientname'] .= "<br>$unpaidbillables";
			}
...
function toggleLockouts() { // 
	$('.lockout').parent().parent().toggle();
	var totalDue = $("#sumNetDue_nonrecurring").attr("due");
	alert($("#sumNetDue_nonrecurring").html());
	if(totalDue) {
		var out = $("#sumNetDue_nonrecurring").html();
		var lockTot = lockoutTotal();
		out += " - locked out total ($ "
				+lockTot.toFixed(2)
				+") = $ "
				+(totalDue - lockTot).toFixed(2);
		$("#totalsWithLockouts").html(out);
	}
}

function lockoutTotal() {
	var total = 0;
	$('.lockout').parent().parent().each(
		function(i, el) {
			var td = $(el).find("td:eq(5)");
			var span = $(td).find("span:eq(0)");
			var due = parseFloat(span.attr('due'));
			total += due;
		}
	);
	return total;
...
function summarizeSelectedInvoices(section) {
	var sels = getSelections(section, 'Please select one or more invoices to summarize.');
	var excludes;
	if(sels.length > 0) excludes = getExcludedPriorUnpaidSelections(sels);
	if(sels.length == 0) return;
	var lookahead = document.getElementById('lookahead_'+section).value;
	...
	if(templateSelect != null && typeof templateSelect != 'undefined')
			args += '&templateid='+templateSelect.options[templateSelect.selectedIndex].value;
	var url = 'billing-statement-email.php?summary=1&ids='+sels+args;
	openConsoleWindow("summarywin", url,600,500);

	//ajaxGetAndCallWith('billing-statement-email.php?summary=1&ids='+sels+args, openSummaryReport, null);
}
