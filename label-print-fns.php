...
if(0 && mattOnlyTEST()) {
 ...
 return array
        (
            'experimental' => 1,
            'label' => '1.25 in X 0.5 in - DYMO D1 Half Inch Strip labels',
            'stockFormat' => 'dymoD1_halfinch',
            'units' => 'cm',
            'borderwidth' => 0.0,
            'borderheight' => 0.0,
            'barheight' => 1.35, // 1.55,
...
function printKeyLabels($keys, $stockFormat='fullSheetFormat') {
	...
	else if($stockFormat == 'dymoD1_halfinch') {
		printStripLabelsBySpec(dymoD1_halfinch_PDF(), 'specDymoHalfInch', $spec, $keys); // 'dymoD1_HalfInchStrip'
	}
}
...
//if(mattOnlyTEST()) {print_r($spec);exit;}	
	...
	$fontSize = $key['bin'] && (strlen($key['bin']) >= 12) ? 5: 6; //$fonts[0] : $fonts[1];
	$prefix = ''; //strlen($key['bin']) >= 12 ? ' ' : ' Hook: ';
	$binLabel = trim($prefix.$key['bin']) ? $prefix.$key['bin'] : '*';
	//$binLabel .= " ".print_r($pdf->ez['topMargin'], 1);
	
	$pageHeight = cm(1.27);
	$margins = array('top'=>$pdf->ez['topMargin'], 'left'=>$pdf->ez['leftMargin'], 'bottom'=>$pdf->ez['bottomMargin'], 'right'=>$pdf->ez['rightMargin']);
	
	
	$pdf->y = $pageHeight-$margins['top'];
	//$pdf->y = $top; // $top;
	...
	//$img = imagecreatefromjpeg(barCodeImageFile($keyLabel));
	...
	$style = /*1+*/ 2 + 8 + 64  + 128; //BCS_TRANSPARENT | BCS_ALIGN_LEFT | BCS_IMAGE_JPEG | BCS_DRAW_TEXT
	$width = 120; //round(120 * 1.18);
	$height = 60;
	//createBarcodeImage($code, $style, $width, $height, $file=null, $type='C128A', $xres=1, $font=5, $quality=null)
	$barCodeObj = createBarcodeImage($keyLabel, $style, $width, $height, $file=null, $type='C128A', $xres=1, $font=5);
	$img = $barCodeObj->getImg();
	//$img = mycrop($img , array(0, 16, 113, 40)); // imagecrop avail only in PHP 5.5 and later
	$img = mycrop($img , array(0, 16, 134, 40)); // imagecrop avail only in PHP 5.5 and later
	$tmpFile = tmpfile();
	$tmpfilename = stream_get_meta_data($tmpFile);
	$tmpfilename = $tmpfilename['uri'];
	//imagejpeg($img, $tmpfilename, 100);
	imagepng($img, $tmpfilename, 0);
	if($barCodeObj) {
		$barCodeObj->DestroyObject();
		unset($barCodeObj);  /* clean */
	}
...
//echo "$tmpfilename"; exit;
	...
	$labelTop = $margins['bottom']*.75; //0-cm(.10); //$top + ($spec['topadjustment'] ? cm($spec['topadjustment']) : 0);//!$_SESSION['staffuser'] ? $top : $top - cm(.1);
	//$pdf->addJpegFromFile($tmpfilename , $margins['left'], $labelTop, cm($spec['barwidth']), cm($spec['barheight'])); //.665
	$pdf->addPngFromFile($tmpfilename , $margins['left'], $labelTop, cm($spec['barwidth']), cm($spec['barheight'])); //.665
	$pdf->y = 0;
...
{
    $dest = imagecreatetruecolor($rect[2], $rect[3]);
    imagecopy(
        $dest,
        $src,
        0,
...
function specSheetFormat(&$pdf, $key, $left, $top, $spec=null) {
	...
	$prefix = strlen($key['bin']) >= 12 ? ' ' : ' Hook: ';
	$binLabel = trim($prefix.$key['bin']) ? $prefix.$key['bin'] : '*';
	$labelTop = $top + ($spec['topadjustment'] ? cm($spec['topadjustment']) : 0);//!$_SESSION['staffuser'] ? $top : $top - cm(.1);
	$pdf->y = $labelTop;
//print_r($spec);exit;	
...
//specSheetFormat(&$pdf, $key, $left, $top, $spec=null)	
	foreach($keys as $index => $key) {
		call_user_func_array($labelFunction, array(&$pdf, $key, 
																										$pdf->ez['leftMargin'], 
																										$pdf->ez['pageHeight'],
...
function createBarcodeImage($code, $style, $width, $height, $file=null, $type='C128A', $xres=1, $font=5, $quality=null) {
  ...
  if ($obj) {
			if($file) {
				...
				$obj->SetFont($font);   
				$obj->DrawObject($xres);
				if($quality) $obj->SetQuality($quality);
				$obj->FlushObject();
				$obj->DestroyObject();
				unset($obj);  /* clean */
			}
			else { // return Barcode Object, which can be destroyed later
				$obj->SetFont($font);   
				$obj->DrawObject($xres);
			  if (($obj->getStyle() & BCS_BORDER)) {
						 $obj->DrawBorder();
				}				       
				return $obj;
			}
...
//////////////////////////////////////////////////
function dymoD1_halfinch_PDF() {
	// full height: 1.27 cm
	// printable height: 0.857255
	// margins (top/bottom): 0.206375
	$pageWidth = cm(3.175); // 1.25 inches
	$pageHeight = cm(1.27);//cm(.79375); // 0.5 inches 1.27
  $pdf =& new Cezpdf( array(0,0,$pageWidth,$pageHeight) , 'landscape' );
  $margins = array(cm(0.206375), cm(0.206375), cm(0.206375), cm(0.3)); //ezSetMargins($top,$bottom,$left,$right);
	//$pdf->ezSetMargins($margins[0], $margins[1], $margins[2], $margins[3]);
	$pdf->ezSetCmMargins($top=.206375,$bottom=.206375,$left=.206375,$right=.206375);
  return $pdf;
}

function dymoD1_halfinch_PDFOLD() {
	$pageWidth = cm(3.175); // 1.25 inches
	$pageHeight = cm(.857255);//cm(.79375); // 0.5 inches 1.27
  $margins = array(cm(0.0), cm(0.0), cm(0.0), cm(0.3)); //ezSetMargins($top,$bottom,$left,$right);
  $pdf =& new Cezpdf( array(0,0,$pageWidth,$pageHeight) , 'landscape' );
	$pdf->ezSetMargins($margins[0], $margins[1], $margins[2], $margins[3]);
  return $pdf;
}
...
function dymoD1_HalfInchStrip(&$pdf, $key, $spec) { // NOT USED!
if(mattOnlyTEST()) {print_r($spec); exit;}	
	// left margin (beyond label) is .75cm
  $pdf->y = $pdf->ez['pageHeight'] - $pdf->ez['topMargin'];
	...
	label($key, $pdf, $pdf->y, $pdf->ez['leftMargin'], $spec);
	$pdf->y = $y;
	if(isset($key['bin'])) $pdf->ezText($prefix.$key['bin'] , $fontSize );
