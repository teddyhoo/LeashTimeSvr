...
// THIS FILE IS INCLUDED (REQUIRED) BY  client-sched-makerV2.php

if($_POST && isset($_POST['servicecode_1_1'])) {
	
	if($action == 'schedule') {  // BACK TO STEP 1
		include "client-sched-makerV2.php";
		exit;
	}
...
$windowFrame = 
	$mobileclient ? "mobile-frame-client.php" // mobileclient set when included from schedule maker
								: "frame-client.html";
$windowFrameEnd = 
	$mobileclient ? "mobile-frame-client-end.php" // mobileclient set when included from schedule maker
								: "frame-end.html";
...
if($error) echo "<font color='red'>$error</font><p>";
	...
	include $windowFrameEnd;
	exit;
}



$stepOneLink = fauxLink('1) Schedule Services', 'scheduleServices()', 1, 'Return to editing your schedule..');
...
if($noTimeTravelAllowed) {
	...
	$stepThreeLink = 
		echoButton('', '3) Submit Request', 'submitSchedule()', 'BigButton', 'BigButtonDown', 1, 'Submit your schedule request.')
		.'<br>or '.echoButton('', 'Quit', 'safeQuit()', null, null, 1);
		
	$detail = 'review schedule';
}
logChange($client, 'clientScheduler', 'm', "Step 2: $detail");

$step1Img = 'art/sched-serv-1-off.jpg';
$step2Img = 'art/sched-serv-2-on.jpg';
$step3Img = 'art/sched-serv-3-off.jpg';
$prevButton = echoButton('', 'Back', 'scheduleServices()', null, null, 1, 'Return to editing your schedule.');
$nextButton = "<p>".echoButton('', 'Submit', 'submitSchedule()', 'BigButton', 'BigButtonDown', 1, 'Submit your schedule request.');
...
<table style='width:100%' border=0>
	...
	<td valign=middle style='padding-right:40px;'><?= $prevButton.$nextButton.$quitButton ?></td>
	<? if(!$_SESSION['preferences']['suppressClientSchedulerPriceDisplay']) { ?>
	<td>Total Price: <span style='color:green;font-size:1.4em;font-weight:bold;'><?= "$currencyMark " ?><span id='totalChargeSpan'><?= number_format($totalCharge, 2) ?></span></span></td>
	<? } ?>
	<td>Visits: <span style='color:green;font-size:1.4em;font-weight:bold;'><span id='totalVisits'><?= $numAppointments ?></span></span></td>
	</tr>
	</table>
<? if($suppressMenu) {
...
$closeLink = "<p><b><a href='javascript:$.fn.colorbox.close();'>Click Here to Continue</a></b>";
...
if(true) $submitButtonInstruction = 
"big Submit button next to the Total Price"; //<p><img src='/art/sched_serv_click_the_submit_button.jpg'><p>
...
$reminder = 
  $noTimeTravelAllowed 
		...
		: "<h2>Remember!</h2><div style='font-size:1.5em'><img src='art/lightning-smile-small.jpg' style='float:right;'>You must click the {$submitButtonInstruction} to send this Schedule Request to us!$closeLink</div>";
?>
...
<tr><td>Visits: <?= $numAppointments ?></td><td>Days without visits: <?= $noVisitDays ?></td>
	<? if(!$_SESSION['preferences']['suppressClientSchedulerPriceDisplay']) { ?>
<td>Price: <?= $currencyMark.number_format($totalCharge, 2) ?><td>
<? } ?>
</tr>
</table>
