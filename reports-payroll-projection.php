<? // reports-payroll-projection.php

require_once "common/init_session.php";
require_once "common/init_db_petbiz.php";
require_once "js-gui-fns.php";
require_once "provider-fns.php";
require_once "client-fns.php";
require_once "service-fns.php";

$failure = false;
// Determine access privs
if($_REQUEST['print']) $auxiliaryWindow = true; // prevent login from appearing here if session times out
$locked = locked('o-#vr');
extract(extractVars('start,end,print,providers,sort,csv,detailsort', $_REQUEST));

		
$pageTitle = "Projected Payroll <font color=red></font>";


if(!$print && !$csv) {
	$breadcrumbs = "<a href='reports.php'>Reports</a> - <a href='reports-payroll.php'>Payroll</a>";	
	include "frame.html";
	// ***************************************************************************
?>
<style>
.dateRow { font-weight: bold; }
.selectedbackground { background: lightyellow; }
</style>

	<form name='reportform' method='POST'>
	<table>
	<tr><td colspan=2>
<?
	calendarSet('For the period starting:', 'start', $start, null, null, true, 'end');
	echo "&nbsp;";
	calendarSet('and ending:', 'end', $end);
	hiddenElement('csv', '');
	
	if(mattOnlyTEST()) {
		$firstDayThisMonthInt = strtotime(date('Y-m-01'));
		echo " ";
		$startM = shortDate(strtotime(date("Y-m-01", strtotime("-1 month", $firstDayThisMonthInt))));
		$endM = shortDate(strtotime(date("Y-m-t", strtotime("-1 month", $firstDayThisMonthInt))));
		$monthLabel = date('M', strtotime($startM));
		fauxLink($monthLabel, "setRange(\"$startM\", \"$endM\")");

		echo " - ";
		$startM = shortDate(strtotime(date("Y-m-01")));
		$endM = shortDate(strtotime(date("Y-m-t")));
		$monthLabel = date('M', strtotime($startM));
		fauxLink($monthLabel, "setRange(\"$startM\", \"$endM\")");
			
		echo " - ";
		$startM = shortDate(strtotime(date("Y-m-01", strtotime("+1 month", $firstDayThisMonthInt))));
		$endM = shortDate(strtotime(date("Y-m-t", strtotime("+1 month", $firstDayThisMonthInt))));
		$monthLabel = date('M', strtotime($startM));
		fauxLink($monthLabel, "setRange(\"$startM\", \"$endM\")");

		
	}
	
?>
	</td></tr>
	<tr><td colspan=2>
<?
$options = array('All Sitters'=>-1);
$options = array_merge(
	$options, 
	array('Active Sitters'=>
				fetchKeyValuePairs("SELECT CONCAT_WS(' ', fname, lname), providerid FROM tblprovider WHERE active=1 ORDER BY lname, fname"),
				'Inactive Sitters'=>
				fetchKeyValuePairs("SELECT CONCAT_WS(' ', fname, lname), providerid FROM tblprovider WHERE active=0 ORDER BY lname, fname")));
labeledSelect('Providers:', 'providers', $providers, $options);
$detailsort = $detailsort ? $detailsort : 'date';
$radioSet = radioButtonSet('detailsort', $detailsort, array('date'=>'date', 'client'=>'client'), $onClick=null, $labelClass=null, $inputClass=null);
echo " Sort payment details by ";
foreach($radioSet as $radio) echo " $radio";
?>
	</td></tr>
	
	</table>
<?
	echoButton('', 'Generate Report', 'genReport()');
	echo "&nbsp;";
	$pstart = date('Y-m-d', strtotime($start));
	$pend = date('Y-m-d', strtotime($end));
	//echoButton('', 'Print Report', "spawnPrinter()");
	//echo "&nbsp;";
	echoButton('', 'Download Spreadsheet', "genCSV()");  // 11/19/2020 dropped pre-req $_SESSION['preferences']['enableProjectedPayrollDownload']) 
?>
	</form>
	<script language='javascript' src='popcalendar.js'></script>
	<script language='javascript' src='check-form.js'></script>
	<script language='javascript' src='common.js'></script>
	<script language='javascript'>
	setPrettynames('start','Starting Date','end','Ending Date');
	function genReport() {
		if(MM_validateForm(
				'start', '', 'R',
				'end', '', 'R',
				'start', '', 'isDate',
				'end', '', 'isDate')) document.reportform.submit();
	}
	function genCSV() {
		if(MM_validateForm(
				'start', '', 'R',
				'end', '', 'R',
				'start', '', 'isDate',
				'end', '', 'isDate')) {
			document.getElementById('csv').value=1;
		  document.reportform.submit();
			document.getElementById('csv').value=0;
		}
	}
	function spawnPrinter() {
		//document.location.href='reports-revenue.php?print=1&start=$pstart&end=$pend&reportType=$reportType'>
		if(MM_validateForm(
				'start', '', 'R',
				'end', '', 'R',
				'start', '', 'isDate',
				'end', '', 'isDate')) {
			var start = escape(document.getElementById('start').value);
			var end = escape(document.getElementById('end').value);
			var providers = document.getElementById('providers');
			providers = providers.options[providers.selectedIndex].value;
			openConsoleWindow('reportprinter', 'reports-payroll-projection.php?print=1&start='+start+'&end='+end+'&providers='+providers+'&sort='+'<?= $sort ?>', 700,700);
		}
	}
	
	function setRange(start, end) {
		document.getElementById('start').value = start;
		document.getElementById('end').value = end;
		genReport();
	}
	

	
<? dumpPopCalendarJS(); ?>
	</script>
<?
} // if(!$print)
else if($csv) {
	header("Content-Type: text/csv");
	header("Content-Disposition: attachment; filename=Payroll-Report.csv ");
	dumpCSVRow('Projected Payroll Report');
	dumpCSVRow("Report generated: ".shortDateAndTime('now', 'mil'));
	dumpCSVRow("Period: $start - $end");
}
else {
	$windowTitle = 'Projected Payroll Report';
	require "frame-bannerless.php";
	fauxLink("<h1 style='display:inline;'>Click Here to Print</h1>", "printThisPage(this);");
	echo "<h2 style='text-align:center'>Payroll Report</h2>";
	echo "<p style='text-align:center'>Report generated: ".shortDateAndTime('now', 'mil')."</p>";
	echo "<p style='text-align:center'>Period: $start - $end</p>";
}
if($start && $end) {
//if(mattOnlyTEST()) {echo "PAY: [{$filter}]".print_r($_POST, 1).'<br>';}
	
	fetchPayments($start, $end, $providers, $sort);
//echo ">>>";print_r($allPayments);	
	
	if($csv) {
		payrollCSV($start, $end, $sort);
		exit;
	}

	else payrollTable($start, $end, $sort);
}
if(!$print && !$csv){
	echo "<br><img src='art/spacer.gif' width=1 height=300>";
// ***************************************************************************
	include "frame-end.html";
}
else if($print) {
?>
	<script language='javascript'>
	function printThisPage(link) {
		link.style.display="none";window.print();
	}
	</script>
<?
}

function fetchPayments($start, $end, $providers, $sort) {
	global $allPayments, $totalPayments, $totalRevs, $providerSorts, $visitCounts;
	$allPayments = array();
	$providerSorts = array();
	$start = date('Y-m-d', strtotime($start));
	$end = date('Y-m-d', strtotime($end));
	$endtime = date('Y-m-d 23:59:59', strtotime($end));
	$filter = $providers && $providers != -1 ? "AND providerptr = $providers" : "";
	
	$sql = "SELECT providerptr, sum(rate + ifnull(bonus,0)) as amount, count(*) as visitcount,
					sum(charge + ifnull(adjustment,0)) as charge,
					if(CONCAT_WS(' ', fname, lname) = ' ', 'Unassigned', CONCAT_WS(' ', fname, lname)) as provider, 
					CONCAT_WS(' ', lname, fname) as sortname
					FROM tblappointment 
					LEFT JOIN tblprovider ON providerid = providerptr
					WHERE canceled IS NULL AND date >= '$start' AND date <= '$end' $filter
					GROUP BY providerptr
					ORDER BY sortname";
	$result = doQuery($sql);
  while($payearned = mysql_fetch_array($result, MYSQL_ASSOC)) {
		$totalPayments += ($amt = $payearned['amount']);
		$totalRevs += $payearned['charge'];
		$clientSorts[$payearned['provider']] = $payearned['sortname'];
		$allPayments[$payearned['providerptr']] = $payearned;
		$visitCounts[$payearned['providerptr']] = $payearned['visitcount'];
//if(mattOnlyTEST()) {print_r($payearned);exit;}	//mysql_fetch_array($result, MYSQL_ASSOC)
//if(mattOnlyTEST()) {print_r($allPayments[$payearned['providerptr']]);exit;}
	}
//if(mattOnlyTEST()) {print_r($allPayments);exit;}
	
	$sql = "SELECT providerptr, sum(rate) as amount, sum(charge) as charge,
					if(CONCAT_WS(' ', fname, lname) = ' ', 'Unassigned', CONCAT_WS(' ', fname, lname)) as provider, 
					CONCAT_WS(' ', lname, fname) as sortname
					FROM tblsurcharge 
					LEFT JOIN tblprovider ON providerid = providerptr
					WHERE canceled IS NULL AND date >= '$start' AND date <= '$end' $filter
					GROUP BY providerptr
					ORDER BY sortname";
	$result = doQuery($sql);
  while($payearned = mysql_fetch_array($result, MYSQL_ASSOC)) {
//echo print_r($payearned, 1).'<br>';		
		$totalPayments += $payearned['amount'];
		$totalRevs += $payearned['charge'];
		$clientSorts[$payearned['provider']] = $payearned['sortname'];
		$payearned['amount'] += $allPayments[$payearned['providerptr']]['amount'];
		$payearned['charge'] += $allPayments[$payearned['providerptr']]['charge'];
		$allPayments[$payearned['providerptr']]= $payearned;
	}
	
	// SUBTRACT DISCOUNTS
	$sql = "SELECT sum(amount)
					FROM `relapptdiscount`
					LEFT JOIN tblappointment ON appointmentid = appointmentptr
					WHERE canceled IS NULL AND date >= '$start' AND date <= '$end' $filter";
	if($discounts = fetchRow0Col0($sql)) {
		$allPayments[-999]= array('provider'=>'~ Discounts ~', 'charge'=>0-$discounts);
		$totalRevs -= $discounts;
	}
	
	$sql = "SELECT providerptr, sum(amount) as amount,
					if(CONCAT_WS(' ', fname, lname) = ' ', 'Unassigned', CONCAT_WS(' ', fname, lname)) as provider, 
					CONCAT_WS(' ', lname, fname) as sortname
					FROM tblothercomp 
					LEFT JOIN tblprovider ON providerid = providerptr
					WHERE date >= '$start' AND date <= '$end' $filter
					GROUP BY providerptr
					ORDER BY sortname";
	$result = doQuery($sql);
  while($payearned = mysql_fetch_array($result, MYSQL_ASSOC)) {
//echo print_r($payearned, 1).'<br>';		
		$totalPayments += $payearned['amount'];
		$clientSorts[$payearned['provider']] = $payearned['sortname'];
		$payearned['amount'] += $allPayments[$payearned['providerptr']]['amount'];
		$payearned['charge'] = 0 +$allPayments[$payearned['providerptr']]['charge'];
		$allPayments[$payearned['providerptr']]= $payearned;
	}
	
	$sql = "SELECT providerptr, sum(amount) as amount, sum(amount) as gratuity, issuedate as date,
					if(CONCAT_WS(' ', fname, lname) = ' ', 'Unassigned', CONCAT_WS(' ', fname, lname)) as provider, 
					CONCAT_WS(' ', lname, fname) as sortname
					FROM tblgratuity 
					LEFT JOIN tblprovider ON providerid = providerptr
					WHERE issuedate >= '$start' AND issuedate <= '$endtime' $filter
					GROUP BY providerptr
					ORDER BY sortname";
	$result = doQuery($sql);
  while($payearned = mysql_fetch_array($result, MYSQL_ASSOC)) {
//echo print_r($payearned, 1).'<br>';		
		$totalPayments += $payearned['amount'];
		$providerptr = $payearned['providerptr'];
		$clientSorts[$payearned['provider']] = $payearned['sortname'];
		$payearned['amount'] += $allPayments[$providerptr]['amount'];
		$payearned['charge'] += $allPayments[$providerptr]['charge'];
		$payearned['gratuity'] += $allPayments[$providerptr]['gratuity'];
		$allPayments[$providerptr]= $payearned;
	}
//if(mattOnlyTEST()) {print_r($allPayments);exit;}	//mysql_fetch_array($result, MYSQL_ASSOC)
	return $allPayments;
}

function payrollCSV($start, $end, $sort) {
	require "pay-fns.php";
	global $allPayments, $totalPayments, $clientNames, $clientLocs, $services, $surchargeTypes, $hours;
	$clientNames = fetchKeyValuePairs("SELECT clientid, CONCAT_WS(' ', fname, lname, CONCAT('[',clientid,']')) FROM tblclient");
	$clientLocs = fetchAssociationsKeyedBy("SELECT clientid, city, zip FROM tblclient", 'clientid');
	$surchargeTypes = fetchKeyValuePairs("SELECT surchargetypeid, label FROM tblsurchargetype");
	$services = fetchKeyValuePairs("SELECT servicetypeid, label FROM tblservicetype");
	$hours = fetchKeyValuePairs("SELECT servicetypeid, hours FROM tblservicetype WHERE hours IS NOT NULL");
	$providers = getProviderNames();
	$columns = explodePairsLine('date|Date||provider|Sitter||amount|Amount||charge|Revenue||transactionId|Client / Transaction ID||tod_ending|Ending or Time of Day||service|Service||servicecode|Service Code||hours|Hours||note|Notes||city|City||zip|Postal Code');
if(!$_SESSION['staffuser']) unset($columns['hours']);
	dumpCSVRow($columns);
	//$paymentTypes = explodePairsLine('regular|Regular||adhoc|Special');
	foreach($allPayments as $prov => $payment) {
		//$row = $payment;
		$row['date'] = '~~~';;
		$row['provider'] = $providers[$prov];
		$row['amount'] = $payment['amount'];
		$row['transactionid'] = $payment['transactionid'];
		$row['tod_ending'] = $payment['enddate'] ? shortDate(strtotime($payment['enddate'])) : '';;
		$row['service'] = '~~PAYMENT~~';
		$row['servicecode'] = '';
		$row['note'] = $payment['note'];
		dumpCSVRow($row);
		global $prov;
		$prov = $payment['providerptr'];
		payProjectionDetailCSV($prov, $start, $end);
	}
}

function payProjectionDetailCSV($prov, $start, $end) {
	global $csv, $clientLocs;
	$detailsort = 'date';
	$csv = 1;
	include "payroll-projection-detail.php";
}

function payProjectionDetailCSV_OLD($paymentid, $providername) {
	global $clientNames, $services, $surchargeTypes, $hours;
	$payables = getPaymentPayables($paymentid);
	foreach($payables as $payable) {
		$type = $payable['itemtable'];
		$itemptr = $payable['itemptr'];
		$row = array();
		if($type == 'tblappointment') {
			$item = fetchFirstAssoc("SELECT * FROM tblappointment WHERE appointmentid = $itemptr LIMIT 1");
			$row['date'] = shortDate(strtotime($item['date']));
			$row['provider'] = $providername;
			$row['amount'] = $item['rate']+$item['bonus'];
			$row['transactionId'] = $clientNames[$item['clientptr']];
			$row['tod_ending'] = $item['timeofday'];
			$row['service'] = $services[$item['servicecode']];
			$row['servicecode'] = $item['servicecode'];
			$hoursVal = $hours[$item['servicecode']] == '00:00' ? '' : $hours[$item['servicecode']];
if($_SESSION['staffuser']) $row['hours'] = $hoursVal;
		}
		else if($type == 'tblsurcharge') {
			$item = fetchFirstAssoc("SELECT * FROM tblsurcharge WHERE surchargeid = $itemptr LIMIT 1");
			$row['date'] = shortDate(strtotime($item['date']));
			$row['provider'] = $providername;
			$row['amount'] = $item['rate']+$item['bonus'];
			$row['transactionId'] = $clientNames[$item['clientptr']];
			$row['tod_ending'] = $item['timeofday'];
			$row['service'] = 'Surcharge: '.$surchargeTypes[$item['surchargecode']];
			$row['servicecode'] = $item['surchargecode'];
		}
		else if($type == 'tblothercomp') {
			$item = fetchFirstAssoc("SELECT * FROM tblothercomp WHERE compid = $itemptr LIMIT 1");
			$row['date'] = shortDate(strtotime($item['date']));
			$row['provider'] = $providername;
			$row['amount'] = $item['amount'];
			$row['transactionId'] = '';
			$row['tod_ending'] = '';
			$row['service'] = 'Other comp: '.$item['comptype'];
			$row['servicecode'] = '';
		}
		else if($type == 'tblgratuity') {
			$item = fetchFirstAssoc("SELECT * FROM tblgratuity WHERE gratuityid = $itemptr LIMIT 1");
			$row['date'] = shortDate(strtotime($item['issuedate']));
			$row['provider'] = $providername;
			$row['amount'] = $item['amount'];
			$row['transactionId'] = $clientNames[$item['clientptr']];
			$row['tod_ending'] = '';
			$row['service'] = '';
			$row['servicecode'] = '';
		}
		dumpCSVRow($row);
	}
}

function dumpCSVRow($row) {
	if(!$row) echo "\n";
	if(is_array($row)) echo join(',', array_map('csv',$row))."\n";
	else echo csv($row)."\n";
}

function csv($val) {
  $val = (strpos($val, '"') !== FALSE) ? str_replace('"', '""', $val) : $val;
  $val = (strpos($val, "\r") !== FALSE) ? str_replace("\r", ' ', $val) : $val;
  $val = (strpos($val, "\n") !== FALSE) ? str_replace("\n", ' ', $val) : $val;
	return "\"$val\"";
}

function payrollTable($start, $end, $sort) {
	global $allPayments, $totalPayments, $totalRevs, $visitCounts;
	$gratuityColumn = TRUE || dbTEST('tonkapetsitters');
	$columns = explodePairsLine("provider|Sitter||visits|Visits||charge|Revenue||amount|Pay||gratuity| ||net|Business Net"); //||note|Notes
	if(!$gratuityColumn) unset($columns['gratuity']);
	$colClasses = array('amount' => 'dollaramountcell', 'charge' => 'dollaramountcell', 'net' => 'dollaramountcell'); 
	$headerClass = array('amount' => 'dollaramountheader', 'charge' => 'dollaramountheader', 'net' => 'dollaramountheader'); //'dollaramountheader'

	//$columnSorts = array('amount'=>null, 'provider'=>null);
	if($sort) {
		$sort = explode('_', $sort);
		$columnSorts[$sort[0]] = $sort[1];
	}
//if(mattOnlyTEST()) print_r($allPayments);	
	foreach($allPayments as $payment) {
		
		$totalNet += ($net = $payment['charge'] - $payment['amount'] + $payment['gratuity']);
		$row = $payment;
		$row['charge'] = dollarAmount($payment['charge']);
		$row['visits'] = $visitCounts[$payment['providerptr']];
		$row['net'] = dollarAmount($net);
		$gratuityIncluded = "Pay includes ".dollarAmount($payment['gratuity'])." in gratuities not deducted from Revenue to calculate Business Net.";
		$row['gratuity'] = $payment['gratuity'] ? "<span style='cursor:pointer;font-weight:bold;' title='$gratuityIncluded' onclick='alert(\"$gratuityIncluded\")'> 	&#128178;</span>"
											: '';
		$totalGratuities += $payment['gratuity'];
		$row['amount'] = fauxLink(dollarAmount($payment['amount']), "payProjectionDetail({$payment['providerptr']})", 1);
		$rows[] = $row;
		$rows[] = array('#CUSTOM_ROW#'=>"<tr><td colspan=4 id='detail_{$payment['providerptr']}' class='selectedbackground'></td></tr>");
	}
	$totalPayments = dollarAmount($totalPayments);
	$totalRevs = dollarAmount($totalRevs);
	$rows[] = array('#CUSTOM_ROW#'=>"<tr>
		<td class='sortableListCell' style='font-weight:bold;text-align:right;'>Totals</td>
		<td class='dollaramountcell' style='font-weight:bold;'>".array_sum($visitCounts)."</td>
		<td class='dollaramountcell' style='font-weight:bold;'>".dollarAmount($totalRevs)."</td>
		<td class='dollaramountcell' style='font-weight:bold;'>".dollarAmount($totalPayments)."</td>"
		.($gratuityColumn ? "<td></td>" : "")
		."<td class='dollaramountcell' style='font-weight:bold;'>".dollarAmount($totalNet)."</td>
		</tr>");
	
	if(!$allPayments) {
		echo "No sitters found.";
		return;
	}
	echo "<p><span class='tiplooksleft'>Click a pay amount once to view supporting details.  Click again to close the detail.</span><p>";
	tableFrom($columns, $rows, 'width=60%', $class=null, $headerClass, $headerRowClass=null, $dataCellClass='sortableListCell', $columnSorts, $rowClasses=null, $colClasses, 'sortClick');
}
?>
<script language='javascript' src='ajax_fns.js'></script>
<script language='javascript'>

function payProjectionDetail(prov) {
	var td = document.getElementById('detail_'+prov);
	var detailsort;
	if(document.getElementById('detailsort_date').checked) detailsort = 'date';
	else if(document.getElementById('detailsort_client').checked) detailsort = 'client';
	if(td.innerHTML) {
		$.fn.removeClass("selectedbackground");
		td.innerHTML = '';
	}
	else {
		$.fn.addClass("selectedbackground");
		$('.BlockContent-body').busy("busy");
		ajaxGetAndCallWith("payroll-projection-detail.php?start=<?= date('Y-m-d', strtotime($start)) ?>&end=<?= date('Y-m-d', strtotime($end)) ?>&prov="+prov+"&detailsort="+detailsort, fillInDetail, 'detail_'+prov);
		//ajaxGet("payroll-projection-detail.php?start=<?= date('Y-m-d', strtotime($start)) ?>&end=<?= date('Y-m-d', strtotime($end)) ?>&prov="+prov, 'detail_'+prov);
	}
}

function fillInDetail(divid, html) {
	document.getElementById(divid).innerHTML = html;
	$('.BlockContent-body').busy("hide");
}

function sortClick(sortKey, direction) {
	var start = '<?= $start ?>';
	var end = '<?= $end ?>';
	var providers = document.getElementById('providers');
	providers = providers.options[providers.selectedIndex].value;
	document.location.href='reports-payroll.php?sort='+sortKey+'_'+direction
		+'&start='+start+'&end='+end+'&providers='+providers;
}
</script>