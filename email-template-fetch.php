...
$locked = locked('o-');
if(strpos($_REQUEST['id'], 'O_') === FALSE)
	$template = fetchFirstAssoc("SELECT * FROM tblemailtemplate WHERE templateid = {$_REQUEST['id']} LIMIT 1");
else $template = fetchOrganizationTemplate($_REQUEST['id']);

if(FALSE && $template['salutation']) {
	$body = $template['salutation'];
	if($template['personalize'] && (strpos($template['salutation'], '#') === FALSE)) $body .= " #RECIPIENT#,";
	$body .= "\n\n";
}
