...
function visitsTable($visits, $action, $focus) {
	...
	foreach($visits as $apptid => $appt) {
		...
		$rowClasses[] = $rowClass;
		$serviceLabel = labelFor($appt['service'], $cbid);
		$futureVisit = strtotime($appt['date'].' '.$appt['starttime']) > time();
		if($futureVisit && !$appt['canceled']) {
			$tooLateToCancel = $_SESSION['preferences']['cancellationDeadlineHours'];
			if(!$alreadyWarned && $tooLateToCancel && time() + (60 * 60 * $tooLateToCancel) > strtotime($appt['date'].' '.$appt['starttime'])) {
				$alreadyWarned = "<b><font color=red>* {$_SESSION['preferences']['cancellationDeadlineWarning']}</font></b>";
				echo $alreadyWarned;
				$serviceLabel = "<font color='red'>*</font> $serviceLabel";
			}
		}	
		$rows[] = array('cb'=>$cb, 'timeofday'=>labelFor($appt['timeofday'], $cbid), 'service'=>$serviceLabel);
