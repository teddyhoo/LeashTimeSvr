<?php
	...
	if ($id && "" . (int) "$id" != $id)
		$id = null;
	if ($redirectToDate) {
		$_SESSION['clientScheduleDateRange'] = date('Y-m-d', strtotime("- 4 days", strtotime($redirectToDate))) . '|' . date('Y-m-d', strtotime("+ 3 days", strtotime($redirectToDate)));
	...
	if ($id && dbTEST('leashtimecustomers') && (staffOnlyTEST()))
		$breadcrumbs .= " - " . fauxLink('Client Biz Details', "descriptionBox()", 1, "Show client business details.");
	if ($id)
		$breadcrumbs .= " - " . fauxLink('Show Nearby Sitters', "openConsoleWindow(\"nearbysitters\", \"client-provider-map.php?pop=1&id=$id\",800,800)", 1, "Open a map") . " <span style='color:red;font-variant:small-caps;'>new</span>";
	if ($id && (staffOnlyTEST() || $_SESSION['preferences']['offerNearbyClientsMap']))
		$breadcrumbs .= " - " . fauxLink('Show Nearby Clients', "openConsoleWindow(\"nearbyclients\", \"client-cluster-map.php?pop=1&id=$id\",800,800)", 1, "Open a map") . " <span style='color:red;font-variant:small-caps;'>beta</span>";
	...
	if ($savedClient && !$savedClient['active']) {
		if (TRUE || staffOnlyTEST())
			$deactivationDate = fetchRow0Col0("SELECT time 
					FROM tblchangelog 
					WHERE itemptr = $id AND itemtable = 'tblclient' AND note = 'Deactivated' 
					ORDER BY time desc LIMIT 1");
		if ($deactivationDate)
			$deactivationDate = '<span style="font-size:0.7em"> - ' . shortNaturalDate(strtotime($deactivationDate)) . "</span>";
		$pageTitle .= " <font color=red>(Inactive$deactivationDate)</font>";
	}
	if ($id && mattOnlyTEST())
		$pageTitle = "<img src='art/snapshot.gif' onclick='openConsoleWindow(\"viewclient\", \"client-view.php?id=$id\", 500, 700)' title='View this client.'> " . $pageTitle;
	if ($id) {
		$scheduleUpdatesAccepted = clientAcceptsEmail($id, array(
			'autoEmailScheduleChanges' => true
		));
	...
	if ($_POST && isset($clientid)) {
		if ($clientid) {
			...
			if ($active != $savedClient['active'])
				setUserActive($savedClient['userid'], $active);
			...
			if ($savedClient['active'] && !$active)
				...
				if ($oldProvider != $defaultproviderptr) {
					if (countCurrentIncompleteClientAppointmentsWithProvider($clientid, $oldProvider) || countCurrentClientPackagesWithProvider($clientid, $oldProvider))
						$gotoServiceReassignmentDialogPage = true;
				}
			}
			if (!$savedClient['active'] && $active)
				$cchanges[] = 'Reactivated.';
			logChange($clientid, 'tblclient', 'm', 'Modified.' . join('', (array) $cchanges));
			if ($continueEditing) {
				if ($continueEditing == 'keyedit')
					$rd = globalURL("key-edit.php?client=$clientid");
				else
					$tab = $continueEditing;
			}
			...
			if ($requestid)
				updateTable('tblclientrequest', array(
					'clientptr' => $newClientId
				), "requestid = $requestid", 1);
			$message = "Client {$_POST['fname']} {$_POST['lname']} has been added.";
			if ($continueEditing) {
				if ($continueEditing == 'another')
					$message .= "  You can add another now.";
				else if ($continueEditing == 'systemloginsetup')
					$tab = 'basic';
				else if ($continueEditing == 'keyedit')
					$rd = globalURL("key-edit.php?client=$newClientId");
				else
					$tab = $continueEditing;
			}
	...
	if ($id) {
		...
		$accountBalance = getAccountBalance($id, true, false);
		...
		if ($_SESSION['ccenabled'] && $id) {
			...
			if ($client['clientid'])
				$cc = getClearPrimaryPaySource($client['clientid']);
			if (!$cc)
		...
		if (staffOnlyTEST())
			$billableButton = "<img src='art/help.jpg' height=20 width=20 onclick='openConsoleWindow(\"billables\", \"client-unpaid-billables.php?id=$id\", 800, 300);'>";
		echo "Account Balance: <span class='accountbalancedisplay' id='globalaccountbalance'>$accountBalance</span>  $billableButton  $ccDisplay";
		echo "<DIV><a href='client-edit5.php?id=$id&tab=basic'>PROFILE no billing</a></div>";
		...
		if ($lastAccessInfo)
			echo "<br>$lastAccessInfo";
		echo "</td></tr></table>";
	}
	if ($warn)
		echo "<script language='javascript'>alert(\"$warn\");</script>";
?>
<table width=700>
<tr><td><?= $message ?></td>
...
<?php
	...
	if ($readOnly) {
		...
		$saveButton       = $id ? echoButton('', 'Save Changes', 'checkAndSubmit("")', 'Button', 'ButtonDown', 'noEcho') : echoButton('', 'Save New Client', 'checkAndSubmit("")', 'Button', 'ButtonDown', 'noEcho');
...
<?php
	$rawBasicCol1Fields   = 'fname,First Name,lname,Last Name,fname2,Alt First Name,lname2,Alt Last Name,email2,Alt Email,' . 'email,Email,active,Active,prospect,Prospect';
	$rawBasicOtherFields  = 'cellphone,Cell Phone,homephone,Home Phone,workphone,Work Phone,cellphone2,Alt Phone,' . 'fax,FAX,pager,Pager,defaultproviderptr,Default Sitter';
	...
	if ($customFields)
		$labelAndIds["custom"] = 'Custom';
	$labelAndIds = array_merge($labelAndIds, array(
		"services" => 'Services',
		"billing" => "Billing",
		"account" => "Account",
	...
	if (!$showAccountTab)
		unset($labelAndIds['account']);
	if (!$showBillingTab)
		unset($labelAndIds['billing']);
	...
	if ($requestid)
		$tablePets = array_values(getProspectPets($request));
	else
		$tablePets = getClientPets($id);
	...
	if ($showAccountTab) {
		...
		if ($client['clientid']) {
			...
			echo "<p><center>This section is used after the new client has been saved.</center><p>";
	...
	if ($client['clientid']) {
		...
		if ($_SESSION['preferences']['enablePrefilledIntakeForm'])
			$rawOptions['Print a Pre-filled Intake Form'] = 'printIntakeForm';
		...
		if (staffOnlyTEST() || dbTEST('pppvb,doggiewalkerdotcom,savinggrace,tailsontrails,tonkatest,tonkapetsitters'))
			$rawOptions['Monthly Billables'] = 'staffMonthlyBillables';
		foreach ($rawOptions as $label => $value) {
			$style = '';
			if (($value == 'viewDiscounts') && !$_SESSION['discountsenabled'])
				continue;
	...
	function saveDiscount($clientid, $arr)
	{
		...
		if ($oldDiscount && !$arr['discount'])
			dropClientDiscount($clientid);
		if ($arr['discount']) {
			$discountId = explode('|', $arr['discount']);
			$discount   = array(
				'discountptr' => $discountId[0],
				'clientptr' => $clientid,
				'memberid' => $arr['memberid']
			);
			if ($oldDiscount && $discount['discountptr'] != $oldDiscount['discountptr'])
				$discount['start'] = null;
			if ($oldDiscount)
				updateTable('relclientdiscount', $discount, "clientptr = $clientid", 1);
			else
				insertTable('relclientdiscount', $discount, 1);
		}
	}
	...
	function interpretInterval($intervalLabel)
	{
		...
		if ($intervalLabel == 'Last Month') {
			...
			if (date('l') == "$weekStart")
				...
				$start = shortDate(strtotime("last $weekStart"));
			$end = shortDate(strtotime("next $weekFinish", strtotime($start)));
		}
		return "$start|$end";
	}
	function valuepacksSection()
{
...
JSCODE;
	...
	function billingTableAndPricesTable($client, $cc = null)
	{
		...
		echo "<tr><td colspan=2>GATEWAY: " . $_SESSION['preferences']['ccGateway'] . "</td></tr>\n";
		$invoiceBy = isset($client['invoiceby']) ? $client['invoiceby'] : null;
		radioButtonRow('Invoice by:', 'invoiceby', $invoiceBy, array(
			'E-Mail' => 'email',
			'Paper Mail' => 'mail'
		));
			...
			if ($client['clientid']) {
				...
				if (achEnabled())
					...
					if (adequateRights('*cm')) {
						echo "<tr><td colspan=2>" . echoButton('', 'Edit Credit Card Info', "openConsoleWindow(\"cceditor\", \"cc-edit.php?client={$client['clientid']}\",500,600)", null, null, 1);
						if (false && $cc)
							echo " " . echoButton('', 'Credit Card Refund', "openConsoleWindow(\"ccrefund\", \"cc-refund.php?client={$client['clientid']}\",420,410)", null, null, 1, "Issue a refund to this Credit Card.");
						echo "</tr><tr><td colspan=2>" . fauxLink('View Electronic Transactions', "openConsoleWindow(\"cctransreport\", \"cc-transaction-history.php?client={$client['clientid']}\",800,410)", 1) . "</tr>";
					}
				}
			}
	...
	function ePaymentTableRows($client)
	{
		if ($client['clientid']) {
			...
			if (getPreference('gatewayOfferACH')) {
				...
				echo "</td></tr>";
				echo "<tr><td colspan=2>" . echoButton('', 'Edit ACH Info', "openConsoleWindow(\"cceditor\", \"ach-edit.php?client={$client['clientid']}\",500,600)", null, null, 1);
				echo "<tr><td colspan=2><hr></td></tr>";
			}
			echo "</tr><tr><td colspan=2>" . fauxLink('View Electronic Transactions', "openConsoleWindow(\"cctransreport\", \"cc-transaction-history.php?client={$client['clientid']}\",800,410)", 1) . "</tr>";
		}
	...
	function unusableFlag($paymentSource)
	{
		...
		if (gatewayConflict($paymentSource))
			return "<br><span style='color:red;font-variant:small-caps;'>Unusable: was set up for " . ($paymentSource['gateway'] ? $paymentSource['gateway'] : 'Another Gateway') . "</span>";
	}
	...
	function dumpClientBasicTab($client)
	{
		...
		foreach ($fields as $key => $label) {
			...
			if (in_array($key, array(
				...
				inputRow($label . ':', $key, $val, null, 'emailInput');
				...
				inputRow($label . ':', $key, $val, $labelClass = null, $inputClass = 'standardInput', null, null, $onBlur);
		}
		echo "</table></td>\n";
		...
		foreach ($fields as $key => $label) {
			...
			if (in_array($key, array(
				...
				inputRow($label . ':', $key, $val);
		}
		echo "</table></td></tr>\n";
		echo "<tr><td>\n";
		selectElement('Veterinary Clinic:', 'clinicptr', $client['clinicptr'], array(), 'clinicChanged(this)');
		fauxLink(' view', "viewClinic()", 0, "View the selected clinic.");
		echo "\n<td>";
		selectElement('Veterinarian:', 'vetptr', $client['vetptr'], array(), 'vetChanged(this)');
		...
		if (!is_array($systemUser))
			$systemUser = null;
		$args = array(
			'roleid' => $client['clientid'],
			'target' => 'systemLoginButton',
			'lname' => $client['lname'],
			'fname' => $client['fname'],
		...
		if ($systemUser)
			$args['userid'] = $systemUser['userid'];
		$args['role'] = 'client';
		foreach ($args as $k => $v)
			$argstring[] = "$k=" . urlencode($v);
		$argstring             = join('&', $argstring);
		$systemLoginEditButton = $systemUser ? $systemUser['loginid'] . ($systemUser['active'] ? '' : " (inactive login)") : 'Set Client Login';
		...
		if ($_SESSION['serviceagreementsenabled']) {
			if (!$client['userid'])
				...
				if (!$agreementSigned)
					...
					if ($_SESSION['preferences']['offerManagerLinkToClientsAgreement']) {
						$agreementStyle = "style='text-decoration:underline;cursor:pointer;";
					}
					$agreementSigned = "<span $agreementClick title='Signed " . longestDayAndDateAndTime(strtotime($agreementSigned['agreementdate'])) . "'" . " $agreementStyle'>" . ($agreementSigned['agreementptr'] < 0 ? '<b>(Global)</b> ' : '') . $agreementSigned['label'] . "</span>";
				}
			}
			if ($agreementSigned != "No") {
				$printerIcon     = '<span class="fontSize1_2em">&#128438; </span>';
				$agreementSigned = fauxLink("$printerIcon$agreementSigned", "openConsoleWindow(\"viewagreement\", \"client-agreement-signed.php?id=$id\", 700, 700)", $elementnoEcho = true, $elementtitle = 'View agreement', $elementid = null, $elementclass = '', $elementstyle = 'cursor:pointer');
			}
			...
			if ($id && dbTEST('leashtimecustomers')) {
				...
				$trialDiscontinuationURL   = !$trialDiscontinuationBizId ? 'NO GARAGE GATE CODE SET.' : getTrialDiscontinuationFormURL($trialDiscontinuationBizId);
				labelRow('Deactivation Form URL', '', echoButton('', 'Show Deactivation Form URL', $onClick = "$(\"#trialDiscontinuationURL\").css(\"display\",\"inline\")", $class = '', $downClass = '', $noEcho = true, $title = 'Generate a URL to a Discontinue form to send to a client business.'), null, null, null, null, 'raw');
		...
		if ($_SESSION["officenotes_logbook_enabled"]) {
			...
			if (!$id)
				echo "Please save this client first before entering Office Notes";
			else {
	...
	function dumpHomeInfoTab($client)
	{
		...
		if ($_SESSION['secureKeyEnabled'])
			$pairs .= '||nokeyrequired|No Key Required';
		foreach (explodePairsLine($pairs) as $key => $label) {
			if ($key == 'nokeyrequired')
				checkboxRow($label . ':', $key, $client[$key]);
			else
		...
		if (staffOnlyTEST() && dbTEST('dogslife')) {
			...
			$clientProps = $client['clientid'] ? getClientPreferences($client['clientid'], $keys = array_keys($extraCodes)) : array();
			$toggles     = array();
			foreach ($extraCodes as $key => $val) {
				if ($clientProps[$key])
					inputRow($extraCodes[$key] . ':', "prop_$key", $val, null, 'streetInput');
				else {
					inputRow($extraCodes[$key] . ':', "prop_$key", null, null, 'streetInput', "row_$key", 'display:none');
					$toggles[] = fauxLink($extraCodes[$key], "this.style.display=\"none\";$(\"#row_$key\").toggle()", 'noEcho', !$title, "link_$key", null, "display:inline");
				}
			}
			if ($toggles) {
		...
		for ($i = 0; $i < count($raw) - 1; $i += 2)
			$fields[$raw[$i]] = $raw[$i + 1];
		echo "<td style='vertical-align:top'><table width=100%>\n<tr><td colspan=2>Alarm</td></tr>";
		foreach ($fields as $key => $label) {
			if ($key != 'alarminfo')
				inputRow($label . ':', $key, $client[$key]);
			else {
				echo "\n<td valign=top colspan=2>";
	...
	function clientKeyTable($client)
	{
		...
		if (false)
			...
			$printKeyButton = "$keyId " . echoButton('', 'Edit / Print Labels', 'printKeyLabels()', '', '', true, 'Click here to save the client and print key labels.');
			keyTable($key, $printKeyButton);
			if ($client['clientid'] && $_SESSION['preferences']['enableKeyOfficeNotes']) {
	...
	if ($rawServiceTypeFields) {
		$serviceTypePairs = explode('|||', $rawServiceTypeFields);
	...
	if ($prettyServices)
		$allRawNames .= "," . join(",", array_map('addslashes', (array) $prettyServices));
	...
	if ($_SESSION['referralsenabled']) {
		$referralCats = getReferralCategories($_SESSION['preferences']['masterPreferencesKey'], $inactiveAlso = 1);
...
<?php
	if (function_exists('dumpCustomFieldJavascript'))
		dumpCustomFieldJavascript($id);
	if (function_exists('dumpNRSectionJS'))
		dumpNRSectionJS();
	echo <<<NRS
...
<?php
	if (isset($clienteditalert))
		echo "alert(\"$clienteditalert\");\n";
?>
setPrettynames(<?= $prettyNames ?>);	


function checkAndSubmit(continueEditing, justcheck) {
	if(typeof justcheck=="undefined" || justcheck==null || justcheck==0 || justcheck=="") 
		justcheck = false;
		$('input[type="button"]').prop('disabled', true); // prevent dup clients
	<?php if ($_SESSION['referralsenabled']) echo "var referralMessage = referralIsIncomplete(true);\n";?>
	...
	if(!MM_validateForm(
		...
		<?= $serviceTypeConstraints ?>
		<?php if ($_SESSION['referralsenabled']) echo ",\n referralMessage, '', 'MESSAGE'";?>
		<?php if ($_SESSION['discountsenabled']) echo ",\n referralMessage, '', 'MESSAGE'";?>
	)) {
...
if (tableExists('tblconfirmation') && $_GET['notifytime']) {
	...
	if ($interval < 2 || $_SESSION['clientEditNotifyToken'] == $_GET['notifytime']) {
