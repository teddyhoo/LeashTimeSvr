...
//echo "Headers: ".print_r($row, 1)."<p>titleColumn: $titleColumn<hr>";
...
//echo "ROW (".rowAtHeader($row, 'Gross')." = $grossFloat: ".("$grossFloat" == rowAtHeader($row, 'Gross'))."): ".print_r($row, 1)."<br>";			
			if(!($title = $row[$titleColumn]) // Subject is usually "LeashTime Service (@...)"
				...
				&& rowAtHeader($row, 'Type') != "Payment Received"
...
//echo "ROW[id: $id] [title: $title]: ".print_r($row, 1)."<br>";			
			$paypal[$id][] = 
				array(
					...
					'payer'=>(rowAtHeader($row, 'Name') ? rowAtHeader($row, 'Name').' ('.rowAtHeader($row, 'From Email Address').')' : rowAtHeader($row, 'Type')),
					'title'=>$title,
					'clientid'=>$id);
...
function echoUnknownRow(&$row) {
	...
	$possible = fetchAssociations(
			"SELECT CONCAT_WS(' ', fname, lname) as name, clientid 
				FROM tblclient 
				WHERE active=1 AND (fname = '$payer' OR lname = '$payer')", 1);
	foreach($possible as $client) $clients[] = 
		"{$client['name']} (bal: ".getAccountBalance($client['clientid'], $includeCredits=true, $allBillables=false).")?";
	echo "<tr><td colspan=5 style='color:red;font-weight:bold;'>No LT Client ID supplied for: {$payer} "
				."<span style='color:grey;'>".join(', ', (array)$clients)."</span></td></tr>";
...
//echo "<tr><td colspan=4>".print_r($allRows, 1);	
	...
	foreach($allRows as $row) {
		...
		if(!$row['creditid']) { // PayPal
			...
			$transactionid = "$transactionid <span $buttonbg><img title='Click to record this PayPal payment in LeashTime' src='art/add-surcharge.gif' width=15 height=15
														onclick='openConsoleWindow(\"paypalpaymenteditor\", \"$url\",600,500)'></span>";
		}
