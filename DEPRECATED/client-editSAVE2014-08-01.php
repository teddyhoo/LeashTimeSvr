...
if($id && (staffOnlyTEST() || $_SESSION['preferences']['offerNearbySittersMap'])) $breadcrumbs .= " - ".fauxLink('Show Nearby Sitters', 
																			"openConsoleWindow(\"nearbysitters\", \"client-provider-map.php?pop=1&id=$id\",800,800)", 1, 1)." <span style='color:red;font-variant:small-caps;'>beta</span>";
...
if($client['clientid']) {
	...
	//if($_SESSION["staffuser"] && !$_REQUEST['novd']) {
		echo " ";
			...
			if(staffOnlyTEST()) $rawOptions['STAFF Monthly Billables'] = 'staffMonthlyBillables';						
...
*/			
  ...
  fauxLink(' view', 'viewClinic("vet")', 0, "View the selected vet.");
	...
	if($_SESSION['serviceagreementsenabled']) {
		...
		if(dbTEST('leashtimecustomers') && mattOnlyTEST()) {
			$trialDiscontinuationBizId = fetchRow0Col0("SELECT garagegatecode FROM tblclient WHERE clientid = $id");
			if(!$trialDiscontinuationBizId) $trialDiscontinuationURL = 'NO GARAGE GATE CODE SET.';
			else {
				ob_start();
				ob_implicit_flush(0);
				require "trial-discontinuation-form.php";
				$trialDiscontinuationURL = ob_get_contents();
				ob_end_clean();
			}
			labelRow('Show Client Deactivation Form URL', '', 
				echoButton('', 'Show URL', $onClick="$(\"#trialDiscontinuationURL\").css(\"display\",\"inline\")", $class='', $downClass='', $noEcho=true, $title='Generate a URL to a Discontinue form to send to a client business.')
				."<span id='trialDiscontinuationURL' style='display:none;'>$trialDiscontinuationURL</span>",
...
function optionsAction(el) {
	...
	if(action == 'staffMonthlyBillables') {
		...
		openConsoleWindow("staffMonthlyBillables", "staff-monthly-billables.php?id=<?= $id ?>", 900, 900);
	}
}
