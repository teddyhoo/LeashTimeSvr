...
//if(mattOnlyTEST()) {
		if($_COOKIE["LEASHTIMEROLE"]) {
			header("Location: https://{$_SERVER["HTTP_HOST"]}/login");
			exit;
		}
//}
	if(strpos($_SERVER["HTTP_REFERER"], '//leashtime.com')) { // session timed out while in the leashtime app
		header("Location: https://{$_SERVER["HTTP_HOST"]}/login");
		exit;
	}
...
// if customer
	...
	if(usingMobileClientApp()) // in init_session.php
		$url = "home-client-mobile.php";
	else $url = "homepage-client.php";
  include $url;
}
...
// unusual cases
if($role == 'T') {
	// solveras-client-cc-pay.php: T == TEMPCLIENT
	// end the session
	session_unset();
	session_destroy();
...
if($role == 'c-temp') {
	// client-cc-payNEW.php, client-cc-payOLD.php: TEMPCLIENT
	// end the session
	session_unset();
	session_destroy();
  header("Location: ".globalUrl("index.php"));
