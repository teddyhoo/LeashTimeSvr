...
if($id && staffOnlyTEST()) $breadcrumbs .= " - ".fauxLink('Show Nearby Sitters', 
																			"openConsoleWindow(\"nearbysitters\", \"client-provider-map.php?pop=1&id=$id\",800,800)", 1, 1);

if($id && $_SESSION["flags_enabled"]) {
	require_once "client-flag-fns.php";
	//$flagPanel = clientFlagPanel($id, $officeOnly=false);
	...
	$flagPanel = clientFlagPanel($id, $officeOnly=false, $noEdit=false, $contentsOnly=false, $onClick=null, $_SESSION['preferences']['betaBillingEnabled']);
}

$pageTitle = $savedClient ? "Client: {$savedClient['fname']} {$savedClient['lname']} $flagPanel" : "New Client";
if($savedClient && !$savedClient['active']) $pageTitle .= " <font color=red>(Inactive)</font>";
...
//print_r($_POST);exit;
	...
	if($gotoServiceReassignmentDialogPage) {
...
if($id) {
	...
	if($_SESSION['ccenabled'] && $id) {
		...
		if(!$cc) $ccDisplay = 'None on file.';
			...
			foreach(getAllCardTypes() as $type) if($type['label'] == $cc['company']) $image = $type['img'];
...
if($readOnly) {
	...
	$saveButton = $id ? echoButton('', 'Save Changes', 'checkAndSubmit(this, "")', null, null, 'noEcho')
										: 	echoButton('', 'Save New Client', 'checkAndSubmit(this, "")', null, null, 'noEcho');
	$saveAndAddButton = !$id ? echoButton('', 'Save & Add Another', 'checkAndSubmit(this, "another")', null, null, 'noEcho') : '';
}

function customSaveButton($tabName) {
	global $saveButton;
	return str_replace('checkAndSubmit(this, "")', "checkAndSubmit(this, \"$tabName\")", $saveButton);
}
...
if($showAccountTab) {
	...
	if($client['clientid']) historySection();
...
if($client['clientid']) {
	if(!$readOnlyVisits) {
		echoButton('', 'New EZ Schedule', "saveAndRedirect(this, \"service-irregular.php?client={$client['clientid']}\")");
		if(!$_SESSION['preferences']['hideOneDayScheduleAtTop']) {
			echo " ";
			echoButton('', 'New One-Day Schedule', "saveAndRedirect(this, \"service-oneday.php?client={$client['clientid']}\")");
		}
		if(!$_SESSION['preferences']['hideProScheduleAtTop']) {
			echo " ";
			echoButton('', 'New Pro Schedule', "saveAndRedirect(this, \"service-nonrepeating.php?client={$client['clientid']}\")");
		}
	}
	else {
	...
	//if($_SESSION["staffuser"] && !$_REQUEST['novd']) {
		echo " ";
			...
			if(staffOnlyTEST()) $rawOptions['Client Change History'] = 'clientChangeHistory';						
			if(staffOnlyTEST()) $rawOptions['STAFF Visit Change History'] = 'staffVisitChangeHistory';						
...
if(!$id) {
	...
	echoButton('', 'Save and Continue', 'checkAndSubmit(this, "services")');
}
else {
	appointmentsSection();
...
function recurringServicesSection($client) {
	...
	if(!$schedules) {
		if(!$readOnlyVisits) {
			echoButton('', 'New Ongoing Per-visit Schedule', "saveAndRedirect(this, \"service-repeating.php?client={$client['clientid']}\")");
			echo " ";
			if($_SESSION['preferences']['monthlyServicesPrepaid'])
				echoButton('', 'New Fixed Price Monthly Schedule', "saveAndRedirect(this, \"service-monthly.php?client={$client['clientid']}\")");
		}
	}
	else {
...
function historySection() {
	...
	$paymentEditor = mattOnlyTEST() ? "payment-edit-2stage.php" : "payment-edit.php";
  echo <<<JSCODE
...
function openComposer() {
	openConsoleWindow('emailcomposer', 'comm-composer.php?client=$id',500,500);
}
...
//print_r(fetchCol0("SHOW TABLES"));
		if($client['clientid']) {
			if(achEnabled()) 
				...
				if(staffOnlyTEST()) echo "<tr><td colspan=2>".fauxLink('[LT STAFF] Find Transaction by Transaction ID', 'findTransactionByID()', 1)."</tr>";
			}
		}
...
function ePaymentTableRows($client) {
	if($client['clientid']) {
		...
		if(getPreference('gatewayOfferACH')) {
			...
			$ach = getClearACH($client['clientid']);
			$autopay = $ach['autopay'] ? ' [auto]' : '';
			$achDisp = $ach ? "{$ach['bank']} {$ach['acctnum']} {$ach['accttype']}".$autopay : 'No ACH Info on file.';
...
function unusableFlag($paymentSource) {
	...
	$merchGateway = $_SESSION['preferences']['ccGateway'];
	if(($merchGateway == 'Solveras' && $paymentSource['gateway'] != 'Solveras') 
			|| ($merchGateway != 'Solveras' && $paymentSource['gateway'] == 'Solveras'))
	...
	foreach($fields as $key => $label) {
		...
		if(in_array($key, array('active', 'prospect'))) {
			...
			<td><label for='fname'>$label:</label></td><td><input class='standardInput' id='fname' name='fname' 
						value='".safeValue($client['fname'])."' onBlur= autocomplete='off'> <div id='dupnames' style='display:inline;color:darkgreen;text-decoration:underline;'></div></td></tr>\n";
...
*/			
  ...
  fauxLink(' view', 'viewClinic("vet")', 0, "View the selected vet.");
	...
	$systemLoginEditButton = $systemUser 
														...
														: 'Set Client Login';
	$systemLoginEditButton = echoButton('systemLoginButton', $systemLoginEditButton, "editLoginInfo(this, \"$id\", \"$argstring\")", null, null, 1);
	...
	if($_SESSION['serviceagreementsenabled']) {
		if(!$client['userid']) $agreementSigned = "<i>No user login</i>";
			...
			else $agreementSigned = "<span title='Signed ".longestDayAndDateAndTime(strtotime($agreementSigned['agreementdate']))."'"
																	." style='text-decoration:underline;'>".($agreementSigned['agreementptr'] < 0 ? '<b>(Global)</b> ' : '')
																	.$agreementSigned['label']
																	."</span>";
		}
		labelRow('Agreement signed:', '', $agreementSigned, null, null, null, null, 'raw');
		if(staffOnlyTEST()) labelRow('Client set up (staff Only):', '', ($client['setupdate'] ? shortDate(strtotime($client['setupdate'])) : 'unknown'), null, null, null, null, 'raw');
	}
	echo "</table></td><td>";
	...
	if($_SESSION["officenotes_logbook_enabled"]) {
		...
		echo "<td valign=top>Office Notes: ";
		echo "<br><textarea name='officenotes' cols=40 rows=3>$officenotes</textarea></td>";
	}
	echo "</td>";
...
function dumpHomeInfoTab($client) {
	...
	$pairs = 'leashloc|Leash / Pet Carrier Location||foodloc|Food Location||parkinginfo|Parking Info||garagegatecode|Garage / Gate Code||nokeyrequired|No Key Required';
	...
	$directions = isset($client['directions']) ? htmlentities($client['directions']) : '';
...
function chooseBreed(breedfld, typefld) {
	...
	$.fn.colorbox({href:"breeds.php?pettype="+pettype+"&target="+breedfld, iframe:true,  width:"390", height:"470", scrolling: true, opacity: "0.3"});
}
...
function checkAndSubmit(el, continueEditing) {
...
function viewServicesInvoice(starting, ending, exclude) {
	var email = document.getElementById('email').value;
	var clientid = document.getElementById('client').value;
...
function viewBillingServicesInvoice(starting, ending, exclude) {  // BETA BILLING
	var email = document.getElementById('email').value;
	var clientid = document.getElementById('client').value;
	var origincludeall = ''; //document.getElementById('origincludeall') && document.getElementById('origincludeall').value;
	if(exclude == undefined) exclude = 0;
...
function saveAndRedirect(el, redirectUrl) {
	document.clienteditor.rd.value=redirectUrl;
	if(!checkAndSubmit(el)) document.clienteditor.rd.value='';
}
...
function printKeyLabels(el) {
	...
	if(!confirm("This client must be saved before you print key labels.\n "+
		...
		checkAndSubmit(el, 'keyedit');
	}
}

function editLoginInfo(el, clientid, argstring) {
	if(!clientid) {
		if(!confirm("This client has not been saved, but must be saved\nbefore a system login can be set up.\n"+
	                      "Click OK to save the client and continue."))
	     return;
	  else {
			checkAndSubmit(el, 'systemloginsetup');
		}
	}
	else {
...
function showDups(unused, content) {
	...
	if(content) {
		...
		content = content.length+' similar names found.';
	}
...
function optionsAction(el) {
	...
	if(action == 'arrangeMeeting') document.location.href='client-meeting<?= staffOnlyTEST() ? "NEW" : '' ?>.php?clientptr=<?= $id ?>';
