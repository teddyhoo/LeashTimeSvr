...
ASSUMPTION: if packageptr rather than requestid is supplied, then the last visit date is TODAY
...
extract(extractVars('replyto,all,requestid,packageptr,correspid,corresname,spousename,correstable,correspaddr,subject,msgbody,mgrname,starting,ending', $_REQUEST));
...
if($_POST) {
  ...
  // 							'bizptr'=>$bizptr, 'url'=>$redirecturl, 'loginuserid'=>$loginuserid, 'useonce'=>$useonce);
	...
	$homeSafeParam = $requestid ? "requestid=$requestid" : "packageptr=$packageptr";
	$responseUrl = generateResponseURL($_SESSION["bizptr"], $correspondent, "client-home-safe2.php?$homeSafeParam", $systemlogin=true, $expires=date('Y-m-d', strtotime("+ 5 days")), $appendToken=false);
...
//====================================================================
	...
	if(!$error) {  // Save outbound message
		...
		if($requestid) {
			...
			updateTable('tblclientrequest', array('officenotes'=>$note), "requestid = $requestid", 1);
...
hiddenElement('packageptr', $packageptr);
