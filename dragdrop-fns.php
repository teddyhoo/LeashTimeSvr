...
//calendarSet($label, $name, $value=null, $labelClass=null, $inputClass=null, $includeArrowWidgets=true)
	...
	<tr>
	<td valign=top style='padding:0px;padding-botton:2px;margin:0px;margin-botton:2px;'><div id='appointments' onMouseUp='appointmentUp(this, "appointments")' style='border:solid black 1px;height:500px;overflow:auto;'></div></td>
	<td valign=top style='padding-top:0px;'><div id='allappointments' onMouseUp='appointmentUp(this, "allappointments")' style='border:solid black 1px;height:500px;overflow:auto;'></div></td>
	</tr>
	<tr><td colspan=2 id='reassigned' style='padding-top:10px;vertical-align:top;'onMouseUp='appointmentUp(this, "reassigned")'></td></tr></table>
	</form>
FORM;
}

// ********************************
function getAppointmentsFor($prov, $dateRange) {
...
//echo $sql.'<p>'.print_r($appts, 1);exit;
  ...
  if($providerptr) {
    ...
    if(empty($appts)) echo "<tr><td colspan=3 style='text-align:center;font-style:italic;'>None</td></tr>\n";
			...
			if($appt['date'] != $lastDate) echo "<tr><td class='daybanner'>".date('F j l', strtotime($appt['date']))."</td></tr>";
			$lastDate = $appt['date'];
...
function appointmentPets($appt) {
	if($appt['pets'] =='All Pets') {
		require_once "pet-fns.php";
		return getClientPetNames($appt['clientptr'], $inactiveAlso=false, $englishList=false);
	}
	else return $appt['pets'];
}
...
function getProvisionalAppointmentsFor($prov, $dateRange, $exclude=null) {
	global $jobTableWidth;
	$start = date('Y-m-d', strtotime($dateRange[0]));
	$end = date('Y-m-d', strtotime($dateRange[1]));
  ...
  echo "\n<table  class='taskTable' style='padding-bottom:10px;vertical-align:top;'><tr><th class='taskTableProviderCell' colspan=3>Reassign jobs to: ".
...
function reassignAppointmentTo($apptid, $prov, $origprov) {
	$appt = fetchFirstAssoc("SELECT appointmentid, date, timeofday, servicecode, providerptr, clientptr FROM tblappointment  WHERE appointmentid = $apptid LIMIT 1");
	require_once "provider-fns.php";
...
//echo "providerIsOff($prov, {$appt['date']}, {$appt['timeofday']}): ".providerIsOff($prov, $appt['date'], $appt['timeofday']);
	...
	if($prov && providerIsOff($prov, $appt['date'], $appt['timeofday'])
			...
			|| in_array($appt['clientptr'], doNotServeClientIds($prov)))
		return;
...
function displayAppointmentTile($appt, $prov=null, $zone, $provisionalButNotMovable=false) {
  ...
  $originalOwner = lookupProviderName($appt['providerptr']); 
	if($movable)
	  $onMouseDown = "onMouseDown='return appointmentDown(this, {$appt['appointmentid']}, \"$zone\");'";
	else if($provisionalButNotMovable) 
	  $onMouseDown = "onMouseDown='return cancelReassignment({$appt['appointmentid']}, \"$originalOwner\")'";
	else $onMouseDown = "";
	...
	$pets = appointmentPets($appt);
	$pets = $pets ? " ($pets)" : '';
	echo "<tr $onMouseDown $style>
	<td class='task'><table style='border:outset #888855 1px;width:100%;'><tr>
	  <td>{$client['clientname']}$pets<br>{$client['address']}</td><td $styleRight>$service<br>$time</td>
	</tr></table></td></tr>";
}
...
function executeAllReassignments() {
	...
	$appointments = !$apptIds 
		...
		: fetchAssociationsKeyedBy(
			"SELECT appointmentid, servicecode, charge, providerptr, clientptr, recurringpackage, pets 
				FROM tblappointment 
	...
	$reassignmentToken = time()+$_SESSION['auth_user_id'];
	logChange($reassignmentToken, 'reassignment', 'm', '[START] '.count($assignments).' reassigned.');
	foreach($assignments as $assignment) {
		...
		logChange($assignment['appointmentptr'], 'tblappointment', 'm', "Job reassigned from {$appointments[$assignment['appointmentptr']]['providerptr']}.");
		
		
		/*if($_SESSION['surchargesenabled']) {
			require_once "surcharge-fns.php";
			if(!$appointment['recurringpackage']) {
	...
	if($_SESSION['surchargesenabled']) {
		require_once "surcharge-fns.php";
		if($appointment['recurringpackage'])
			reassignExistingAppointmentSurcharges($appointment["appointmentid"]);
		else updateAppointmentAutoSurcharges($assignment['appointmentptr']);
	}
		...
		logChange($reassignmentToken, 'reassignment', 'm', '[END] '.count($assignments).' reassigned.');
