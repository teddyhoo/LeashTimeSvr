...
$locked = locked('o-#vr');
extract(extractVars('start,end,print,mailsetup,csv,provider', $_REQUEST));
if(!$start && !$end) $start = $end = date('m/d/Y');
		
$pageTitle = "Key Events";

$sitters = getAllProviderSelections($availabilityDate=null, $zip=null, $separateActiveFromInactive=true);
if($start && $end) {
	$events = keyLogSection(date('Y-m-d', strtotime($start)), date('Y-m-d', strtotime($end)));
	if(is_string($events)) $message = $events;
}
...
$rows = array();
$columns = explodePairsLine("time|Time||client|Client||key|Key||mod|Event");
if($message) echo "<p class='tiplooks' style='text-align:left'>$message</p>";
else if($events) {
	$events = array_reverse($events);
	foreach($events as $i=> $event) {
		if($date && $date != substr($event['time'], 0, strpos($event['time'], ' ')))
			$rows[] = array('#CUSTOM_ROW#'=>"<tr><td colspan=4 style='border-top:solid gray 1px'>&nbsp;</td></tr>");
		$date = substr($event['time'], 0, strpos($event['time'], ' '));
		$event['key'] = fauxLink(sprintf("%04d", $event['keyid']), "document.location.href=\"key-edit.php?id={$event['keyid']}\"", 1, 'Edit this key');
		$event['client'] = fauxLink($event['client'], "document.location.href=\"client-edit.php?id={$event['clientptr']}\"", 1, 'Edit this client');
		$rows[] = $event;
	}
