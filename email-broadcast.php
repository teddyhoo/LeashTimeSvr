...
$allowTimeOffScheduleTemplatesByToken = 
	$targetType == 'provider' 
	&& (mattOnlyTEST() || dbTEST('doggiewalkerdotcom')// || $_SESSION['preferences']['enableVisitSheetEmailBrodcast']
			); ///*mattOnlyTEST() ||*/ dbTEST('auntieemspetsitting,dogslife');  // allow any schedule with #SCHEDULE# token to include a schedule
...
CHIMP;
	...
	if($allowTimeOffScheduleTemplatesByToken) {
		$timeOffScheduleTokenFound = strpos($message, '#TIMEOFF_SCHEDULE#') !== FALSE;
		$targettype = $template['targettype'];
	}
...
//logError("Target: ".print_r($target, 1)."]");
	if(strpos($message, '#GRATUITYLINK#') !== FALSE) {
//logError("Found #GRATUITYLINK#");
		$gratuityLookback = null;
		if(($start = strpos($message, '#GRATUITYLOOKBACK#')) !== FALSE) {
logError("Found #GRATUITYLOOKBACK#");
			if(($end = strpos($message, '#ENDGRATUITYLOOKBACK#')) == FALSE)
				$message = str_replace('#GRATUITYLOOKBACK#', '', $message);
			else {
logError("Found #ENDGRATUITYLOOKBACK#");
				$gratuityLookback = 
					substr($message, 
									$start+strlen('#GRATUITYLOOKBACK#'), 
									$end-($start+strlen('#GRATUITYLOOKBACK#')));
				if(0+$gratuityLookback == $gratuityLookback && $gratuityLookback > 0)
					$gratuityLookback = 0-$gratuityLookback;
				else $gratuityLookback = null;
				$block = substr($message, $start, $end+strlen('#ENDGRATUITYLOOKBACK#')-$start);
logError("BLOCK: $block");
				$message = str_replace($block, '', $message);
					
			}
		}
		
		// ensure client, userid
//logError("Found {$target['userid']}|{$target['clientid']}");
		if(!$target['clientid'] || !$target['userid'])
			$message = str_replace('#GRATUITYLINK#', '', $message);
		else {
			$gratuityLink = "https://leashtime.com/client-cc-pay.php?rcip={$target['userid']}|{$target['clientid']}"
											."&sitters=$gratuityLookback&amount=0&note=Sitter+gratuity";
			$message = str_replace('#GRATUITYLINK#', $gratuityLink, $message);
	...
	$message = str_replace('#CLIENTID#', $target['clientid'], $message);
	$message = str_replace('#ACCOUNTBALANCEORPAID#', 
													html_entity_decode(str_replace('&nbsp;', '', "$accountBalanceOrPaidSub")), 
													$message);
	...
	global $allowTimeOffScheduleTemplatesByToken; if($allowTimeOffScheduleTemplatesByToken) {
		$timeOffScheduleTokenFound = strpos($message, '#TIMEOFF_SCHEDULE#') !== FALSE;
		$targettype = $template['targettype'];
	}
	...
	else if($template['label'] == '#STANDARD - Time Off Schedule' || ($targettype == 'provider' && $timeOffScheduleTokenFound))
		$message = mergeTimeOffSchedule($message, $target);
...
function mergeTimeOffSchedule($message, $target) {
	...
	$message = str_replace("\n\n", "<p>", $message);
	$message = str_replace("\n", "<br>", $message);
	$message = str_replace('#TIMEOFF_SCHEDULE#', $schedule, $message);
	return $message;
}
...
function getTimeOffSchedule($providerid,$start, $end) {
	$toffs = fetchAssociations($sql = 
		"SELECT *, IF(timeofday IS NULL, '', LEFT(timeofday, LOCATE('-', timeofday))) as starttime
			FROM tbltimeoffinstance 
			WHERE providerptr = $providerid 
				AND date >= '".date('Y-m-d', strtotime($start))."'
				AND date <= '".date('Y-m-d', strtotime($end))."'
			ORDER BY date ASC, starttime ASC", 1);
	ob_start();
	ob_implicit_flush(0);
	if(!$toffs) 
		echo "No Time Off Scheduled between ".shortDateAndDay(strtotime($start))
						." and ".shortDateAndDay(strtotime($end));
	else {
		echo "<h2>Time Off</h2><p>From ".shortDateAndDay(strtotime($start))
						." to ".shortDateAndDay(strtotime($end))
						."</p>";
		echo "<table>";
		foreach($toffs as $toff) {
			echo 
				"<tr><td style='font-weight:bold;'>".shortDateAndDay(strtotime($toff['date']))."</td>"
				."<td>".($toff['timeofday'] ? $toff['timeofday'] : 'All day')."</td>"
				."<td style='font-size:0.8em;'>Added ".(shortDate(strtotime($toff['created'])).' '.date('h:i a', strtotime($toff['created'])))."</td>"
				."</tr>";
		}
		echo "</table>";
	}
...
function getClientSchedule($clientid, $start, $end) {
	...
	$appts = fetchAssociationsKeyedBy("SELECT * FROM tblappointment WHERE clientptr = $clientid AND date >= '$start' AND date <= '$end' ORDER BY date, starttime", 'appointmentid');
...
?>
	...
	var isTimeOffSchedule = chosenTemplate == '#STANDARD - Time Off Schedule';
<?= !$allowTimeOffScheduleTemplatesByToken ? '' : 
		"\nisTimeOffSchedule = isTimeOffSchedule || (document.getElementById('message').value.indexOf('#TIMEOFF_SCHEDULE#') != -1);"
?>
														
														
	if(isTimeOffSchedule) {
		formArgs.push('start');formArgs.push('');formArgs.push('R');
		formArgs.push('start');formArgs.push('');formArgs.push('isDate');
		//formArgs.push('start');formArgs.push('NOT');formArgs.push('isPastDate');
		formArgs.push('end');formArgs.push('');formArgs.push('R');
...
?>
<?= !$allowTimeOffScheduleTemplatesByToken ? '' : 
		"\nshouldDisplay = shouldDisplay || (document.getElementById('message').value.indexOf('#TIMEOFF_SCHEDULE#') != -1);"
?>
	var display =  shouldDisplay ? 'block' : 'none'; 
