...
if(!$readOnly) {
	$links = 
		array(
			'print'=>"document.location.href=\"billing-invoice-print.php?ids=$id$args\"",
			'email'=>"javascript:document.location.href=\"billing-invoice-email.php?ids=$id$args&includePayNowLink=\"+(payNowEnabled() && document.getElementById(\"includePayNowLink\").checked ? 1 : 0)",
			'charge'=>"javascript:document.location.href=\"billing-invoice-charge-email.php?amount=\"+document.getElementById(\"amountDue\").value+\"&ids=$id$args&includePayNowLink=\"+(payNowEnabled() && document.getElementById(\"includePayNowLink\").checked ? 1 : 0)",
					);
...
function doOnLoad() {
	payNowClicked();
	<? if($amountDue <= 0) echo "document.getElementById('chargeButton').style.display='none';" ?>
}
