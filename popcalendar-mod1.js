...
//	email  : pinotoy@yahoo.com
	...
	var pathHome = pathHome ? pathHome : '';
	var imgDir = pathHome+'calendarimages/';	// directory for images ... e.g. var imgDir="/img/"
	var dayName = '';
        var coverElement = null;                // element to cover with calendar, trumping fixedX, fixedY
	...
	var selectYearMessage = {
		...
		//es : 'Presione para seleccionar un año',  <== causes problems
	...
	var popcalcrossobj, crossMonthObj, crossYearObj, monthSelected, yearSelected, dateSelected, omonthSelected, oyearSelected, odateSelected, monthConstructed, yearConstructed, intervalID1, intervalID2, timeoutID1, timeoutID2, ctlToPlaceValue, ctlNow, dateFormat, nStartingYear, selDayAction, isPast;
	var visYear  = 0;
	var visMonth = 0;
	...
	*/
	...
	function init() {
		if (!ns4)
		{
			...
			popcalcrossobj=(dom)?document.getElementById('calendar').style : ie? document.all.calendar : document.calendar;
			hideCalendar();
	...
	function hideCalendar() {
		popcalcrossobj.visibility = 'hidden';
    ...
    if(ctlToPlaceValue.attributes['onchange']) eval(ctlToPlaceValue.attributes['onchange'].value); //OLD
    ctlToPlaceValue.dispatchEvent(new Event('change')); // NEW
	}
	...
	function popUpMonth() {
		if (visMonth == 1) {
			...
			if(fixedX != -1) crossMonthObj.left = parseInt(fixedX) + 50 + 'px';
			else crossMonthObj.left = parseInt(popcalcrossobj.left) + 50;
			if(fixedY != -1) crossMonthObj.top = parseInt(fixedY) + 'px';
			else crossMonthObj.top =	parseInt(popcalcrossobj.top) + 26;
	...
	function popUpYear() {
		...
		if (visYear==1) {
			...
			leftOffset = parseInt(popcalcrossobj.left) + document.getElementById('spanYear').offsetLeft;
			if (ie) leftOffset += 6;
			
			if(fixedX != -1) crossYearObj.left = parseInt(fixedX) + 130 + 'px';
			else crossYearObj.left = leftOffset;
			if(fixedY != -1) crossYearObj.top = parseInt(fixedY) + 'px';
			else crossYearObj.top = parseInt(popcalcrossobj.top) + 26;
	...
	function showCalendar(ctl, ctl2, format, lang, past, fx, fy, coverElementArg) {
	...
	function popUpCalendar(ctl, ctl2, format) {
		...
		if (bPageLoaded) {
			if (popcalcrossobj.visibility == 'hidden') {
				...
				if ((tokensChanged != 3) || isNaN(dateSelected) || isNaN(monthSelected) || isNaN(yearSelected)) {
					dateSelected  = typeof(initialDate) != 'undefined' ? initialDate : dateNow;
					monthSelected = typeof(initialMonth) != 'undefined' ? initialMonth : monthNow;
					yearSelected  = typeof(initialYear) != 'undefined' ? initialYear : yearNow;
				}
				...
				if(coverElement != null) {aTag = coverElement; cv = coverElement; extraY = 0}
				else {aTag = ctl; cv = aTag; extraY = cv.offsetHeight + 2}
/*				do {
...
alert(aTag);
				...
				while(aTag.offsetParent && aTag.offsetParent.tagName != 'BODY') {
					aTag     = aTag.offsetParent;
					leftpos += aTag.offsetLeft;
					toppos  += aTag.offsetTop;
				}
				...
				popcalcrossobj.left = ((fixedX == -1) ? cv.offsetLeft + leftpos : fixedX)+'px';
				popcalcrossobj.top = ((fixedY == -1) ? cv.offsetTop + toppos + extraY : fixedY)+'px';

				constructCalendar (1, monthSelected, yearSelected);
				popcalcrossobj.visibility = (dom||ie) ? "visible" : "show";
