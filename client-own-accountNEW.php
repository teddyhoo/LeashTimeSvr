...
if($_SESSION["responsiveClient"]) echo "<script>parent.document.location.href='client-own-account.php';</script>";
...
function editPaymentSource(url) {
	<?= $_SESSION["responsiveClient"] 
		? "lightBoxIFrame(url+'?iframe=1', 410, 770);" 
...
<span style='font-size:8px'>Version 2</span>
...
function historySection() {
	global $initializationJavascript, $gatewayObject;
...
//if($_SESSION['auth_user_id'] == 3421) {$DEBUG = "alert(document.cceditor.elements.length);";}	
	...
	echo ".<span style='display:none';>BANG</span>";
