...
$locked = locked('o-#vr');
extract(extractVars('start,end,print,reportType,csv,refNotesAsCategories', $_REQUEST));

		
$pageTitle = "Revenue Report by Referral Category";
...
<?
	...
	if($_SESSION['preferences']['enableUseReferralNotesAsCats']) {
	echo "&nbsp;";
	labeledCheckbox('[BETA] Treat referral notes as subcategories', 'refNotesAsCategories', $value=$refNotesAsCategories, $labelClass=null, $inputClass=null, $onClick=null, $boxFirst=false, $noEcho=false, $title=null);
	}
?>
	</form>
	...
	function spawnPrinter() {
		...
		if(MM_validateForm(
			...
			openConsoleWindow('reportprinter', 'reports-revenue-referral.php?print=1&start='+start+'&end='+end, 700,700);
		}
	}
...
<?
	...
	header("Content-Disposition: inline; filename=Client-Revenue-by-Referral.csv ");
	dumpCSVRow('Revenue Report by Referral Category');
	dumpCSVRow("Report generated: ".shortDateAndTime('now', 'mil'));
	dumpCSVRow("Period: $start - $end");
}
else {
	$windowTitle = 'Revenue Report by Referral Category';
	require "frame-bannerless.php";
	fauxLink("<h1 style='display:inline;'>Click Here to Print</h1>", "printThisPage(this);");
	echo "<h2 style='text-align:center'>Revenue Report by Referral Type</h2>";
...
if($start && $end) {
	...
	//$refNotesAsCategories = staffOnlyTEST();
	revenuesAndCommissionsByReferralCat($start, $end);
	dropProjectionApptTable();
	dropProjectionSurchTable();
if(!staffOnlyTEST()) refunds($start, $end, 'byZips');
	commissionsAndRevenuesByReferralCatTable($start, $end, false, $csv);
}
if(!$print && !$csv){
