...
function sendReminders($onceADay=0) {
	...
	/*else $reminders = fetchAssociations(
			"SELECT * 
				FROM tblreminder 
				WHERE sendon >= '$datetime'");*/
//if(mattOnlyTEST()) { echo "($date)($dow)($dom)<hr>".print_r($reminders,1)."<hr>".print_r(fetchAssociations(
//			"SELECT * 
//				FROM tblreminder"),1);exit;}				
	foreach($reminders as $i => $reminder) {
		...
		if($reminder['remindercode']) $batches[$reminder['remindercode']][] = $reminder;
 			...
 			$extraFields = array();
//array('requestid','fname','lname','phone','whentocall','email','address','street1', 'street2', 'city','state','zip','pets','note','officenotes','clientptr','providerptr', 'resolved', 'requesttype', 'scope');
