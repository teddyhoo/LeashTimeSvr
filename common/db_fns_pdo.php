...
function doQuery($sql, $showErrors=1) {
	global $pdo, $lastPDOResult;
	if($lastPDOResult) $lastPDOResult->closeCursor();
	try {
		$result = $pdo->query($sql); // PDOStatement
	}
	catch (PDOException $e) {
		// ???
		echo "OIK! ".$e->getMessage();
	}
	$lastPDOResult = $result;
  if(sqlErrorCode($result)) {
    if($showErrors) showSQLError($sql);
  }
  return $result;
}
...
function sqlErrorCode($target=null) {
	global $pdo;
	$target = $target ? $target : $pdo;
	list($sqlstate, $errorcode, $errormsg) = $target->errorInfo();
	return $errorcode;
}
...
function showSQLError($sql) {
	global $db, $pdo;
	list($sqlstate, $errorcode, $errormsg) = $pdo->errorInfo();
	if($_SESSION['displayErrorsLoginSetting'])
		echo "SQL: $sql<p><b>ERROR ($errorcode)</b> $errormsg";
	else {
		echo "Sorry, an error has occurred. [$db] [{$_SESSION['auth_user_id']}] ".date('m/d H:i:s');
	}
	if(!in_array($errorcode, array(1045)) && (TRUE || mattOnlyTEST())) {
		$textbagid = bagText($_SESSION['auth_user_id'].'|'.$errormsg.'|'.$sql, $referringtable='tblerrorlog');
		logErrorO($_SESSION['auth_user_id']."|tbag:$textbagid");
	}
	else logError($_SESSION['auth_user_id'].'|'.sqlErrorMessage().'|'.$sql);
	exit;
}
...
function kval($art_array, $key) {
  if(array_key_exists($key, $art_array)) return "'".pdo_real_escape_string(stripslashes($art_array[$key]))."'";
  return "NULL";
}
...
function val($val) {
  ...
  if($val != null) {
   return "'".pdo_real_escape_string(stripslashes($val))."'";
  }
  return "NULL";
}
...
function insertTable($table, $data, $showErrors=0) {
  ...
  $sql .= join(', ', $data).")";
  if(doQuery($sql, $showErrors)) return sqlInsertID(); 
}
...
if(!function_exists('reconnectPetBizDB')) { // Why'd I do this again?

function reconnectPetBizDB($dbN=null, $dbhostN=null, $dbuserN=null, $dbpassN=null, $force=false) {
	global $dbhost, $dbuser, $dbpass, $db, $pdo;
	if($pdo) { // avoid switching if possible
	...
	if(isset($dbhost) && isset($dbuser) && isset($dbpass) && isset($db)) {
		try{
			...
			$errMessage ="Not able to connect: invalid database username and/or password: ".$e->getMessage();
			echo $errMessage;
		}
	}
...
function mattOnlyAddress() { return '68.225.89.173'; }
...
function getUserByID($userid) {
	...
	list($dbhost1, $db1, $dbuser1, $dbpass1) = array($dbhost, $db, $dbuser, $dbpass);
	require  "common/init_db_common.php";
	$recentUsers[$userid] = fetchFirstAssoc("SELECT userid, bizptr, lname, fname, loginid FROM tbluser WHERE userid = $userid LIMIT 1");
