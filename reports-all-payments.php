...
$pageTitle = "Payment/Credit Report";
...
<?
	...
	dumpCSVRow("Payment/Credit: ".str_replace('-', ' ', $specificType));
	dumpCSVRow("Report generated: ".date('m/d/Y H:i'));
}
else {
	$windowTitle = 'Payment/Credit Report';
	require "frame-bannerless.php";
	fauxLink("<h1 style='display:inline;'>Click Here to Print</h1>", "printThisPage(this);");
	echo "<h2 style='text-align:center'>Payments/Credit Report</h2>";
	echo "<p style='text-align:center'>Report generated: ".date('m/d/Y H:i')."</p>";
}

	$filter = $clients == -1 ? '1=1' : (
									$clients == 0 ? "clientptr IN (SELECT clientid FROM tblclient WHERE active = 1)" : (
									$clients == -2 ? "clientptr IN (SELECT clientid FROM tblclient WHERE active = 0)" : 
									"clientptr IN ($clients)"));
									
	$recent = $_REQUEST['recent'];
	
	$orderBySort = 
		$sort == 'type_desc' ? 'payment DESC, sortname, issuedate' : (
		$sort == 'type_asc' ? 'payment ASC, sortname, issuedate' : (
		$sort == 'date_desc' ? 'issuedate DESC, sortname, payment' : (
		$sort == 'date_asc' ? 'issuedate ASC, sortname, payment' : (
		$sort == 'clientname_desc' ? 'sortname DESC, issuedate, payment' : (
		$sort == 'clientname_asc' ? 'sortname ASC, issuedate, payment' :
		'sortname, issuedate')))));
	
	
	$sql = 
		"SELECT *, CONCAT_WS(',', lname, fname) as sortname 
			FROM tblcredit
	...
	foreach($allpayments as $credit) $clientIds[$credit['clientptr']] = 1;
//print_r($allpayments);	
//echo $sql;	
//print_r(array_keys((array)$clients));	
	/*if(isset($clients[0])) {
		$includeUnknown = '<font color=red>Client Unknown</font>';
		unset($clients[0]);
	}*/
	//echo "BANG!".print_r($clients, 1);
	if($clientIds) $clients = getClientDetails(array_keys((array)$clientIds), null, ($recent ? '' : 'sorted'));
	if($includeUnknown) $clients = array_merge(array(0 => array('clientname'=>$includeUnknown) ), (array)$clientIds);
