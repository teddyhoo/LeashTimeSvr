...
extract(extractVars('replyto,all,client,provider,prospect,correspid,corresname,spousename,clientemail2,correstable,correspaddr,subject,msgbody,mgrname,lname,fname,email,tags,template', $_REQUEST));
...
$pageTitle = $pageTitle ? str_replace('#RECIPIENT#', $corrName, $pageTitle) : "Email to $corrName";
...
$rType = $templatetype ? $templatetype : ($client ? 'client' : ($provider ? 'provider' : ''));
...
?>
<tr><td colspan=2>Substitution tokens: #BIZNAME#, #BIZEMAIL#, #BIZPHONE#, #BIZHOMEPAGE#, #BIZLOGINPAGE#, #MANAGER#, #RECIPIENT#, #FIRSTNAME#, #LASTNAME#, #LOGO#, #LOGINID#, #TEMPPASSWORD#, #CREDITCARD# (clients only), #PETS# (clients only)</td></tr>
</table>
</form>
