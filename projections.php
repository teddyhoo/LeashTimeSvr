...
function detailedRevenuesAndCommissionsByClient($start, $end) {
	//for the stated period collected revs and commissions and counts per service type/surchargetype/misc charge
	// NO PROJECTIONS
	global $fixedPriceMonthlyLabel, $figureTaxes, $includeRevenueFromSurcharges;
	$start = date('Y-m-d', strtotime($start));
	$end = date('Y-m-d', strtotime($end));
	...
	$serviceTypeNames = fetchKeyValuePairs("SELECT servicetypeid, label from tblservicetype");
	$surchargeNames = fetchKeyValuePairs("SELECT surchargetypeid, label from tblsurchargetype");
	$monthlyPackages = join(',', fetchCol0("SELECT packageid FROM tblrecurringpackage WHERE monthly"));
	$sql = "SELECT label, tblappointment.clientptr, lname, fname, 
							IF(tblbillable.billableid IS NULL, 
								tblappointment.charge+IFNULL(adjustment,0), 
								tblbillable.charge - IFNULL(tblbillable.tax, 0)) as revenue,
							rate+IFNULL(bonus, 0) as commission
						FROM tblappointment
						...
						WHERE 
							...
							date >= '$start' AND date <= '$end' "
							.($monthlyPackages ? "AND packageptr NOT IN ($monthlyPackages)" : '')
							."\n ORDER BY lname, fname, label";
	$items = array();
	$result = doQuery($sql);
	while($item = leashtime_next_assoc($result)) {
		$items[$item['clientptr']][$item['label']]['label'] = $item['label'];
		$items[$item['clientptr']][$item['label']]['clientid'] = $item['clientptr'];
		$items[$item['clientptr']][$item['label']]['fname'] = $item['fname'];
		$items[$item['clientptr']][$item['label']]['lname'] = $item['lname'];
		$items[$item['clientptr']][$item['label']]['count'] += 1;
		$items[$item['clientptr']][$item['label']]['revenue'] += $item['revenue'];
		$items[$item['clientptr']][$item['label']]['commission'] += $item['commission'];
	}
							
	if($includeRevenueFromSurcharges) {
		$sql = "SELECT CONCAT('SURCHARGE: ', label) as label, tblsurcharge.clientptr, lname, fname, COUNT(*) as number,
							IF(tblbillable.billableid IS NULL, 
								tblsurcharge.charge, 
								tblbillable.charge - IFNULL(tblbillable.tax, 0)) as revenue,
								rate as commission
						...
						WHERE 
							canceled IS NULL AND
							tblsurcharge.date >= '$start' AND tblsurcharge.date <= '$end'
						ORDER BY lname, fname, label";
		$result = doQuery($sql);
		while($item = leashtime_next_assoc($result)) {
			$items[$item['clientptr']][$item['label']]['label'] = $item['label'];
	...
	$sql = "SELECT 'MISC Charge' as label, amount as revenue, tblothercharge.clientptr, tax as billabletax
					FROM tblothercharge
					LEFT JOIN tblbillable ON itemptr = chargeid AND superseded = 0 AND itemtable = 'tblothercharge'
					WHERE issuedate >= '$start' AND issuedate <= '$end'";
	$result = doQuery($sql);
	...
	unset($items['']); // why do we have a '' clientptr???!!!
	return $items;
}
...
if(TRUE || staffOnlyTEST()) { 
	...
	$sql = "SELECT chargeid, amount, tblothercharge.clientptr, issuedate as date, tax as billabletax, paid
					...
					WHERE issuedate >= '$start' AND issuedate <= '$end'
					ORDER BY date, tblothercharge.clientptr";
	revenueForMiscCharges_ZIP($sql, false);		
	//revenueAndCommissionsForAppts_ZIP($sql, false);
...
//if(!$zip) echo "NO ZIP! ".print_r($appt, 1);
...
function revenueAndCommissionsForSurchargesByDateClientAndProvider($sql, $omitRevenues, $includeTaxes=true) {
	global $revenues, $commissions, $actualRevenues, $totalActualRevenue, $taxableRevenues, $totalTaxableRevenue, 
				 $taxes, $totalRevenue, $totalCommission, $visitCounts;
	static $surchargeNames;
	if(!$surchargeNames) $surchargeNames = 
		fetchKeyValuePairs("SELECT surchargetypeid, CONCAT('Surcharge: ', label) FROM tblsurchargetype");
//echo $sql.'<p>'.print_r(fetchAssociations($sql), 1)	;
  ...
  while($surch = mysql_fetch_array($result, MYSQL_ASSOC)) {
		...
		if(!$omitRevenues) {
			...
			if($includeTaxes) {
				$thisTax = figureTaxForAppointmentInSet($surch, $clientTaxRates);
				if($thisTax != 0 && $thisTax != 0.0) {
					$totalTaxableRevenue += $rev;
					$taxableRevenues[$monthYear][$surchargeName] += $rev;
				}
				$taxes[$monthYear][$client][$provider] += $thisTax;  // $taxes is not global?!
			}
...
function revenueForMiscCharges_ZIP($sql, $includeTaxes=true) { // <== NOT NEEDED(?)
	global $revenues, $actualRevenues, $totalActualRevenue, $totalRevenue, $zips;
	$zips = $zips ? $zips : getClientZips();
...
//echo $sql.'<p>'.print_r(fetchAssociations($sql), 1)	;
	...
	$chargeCount = 0;
  while($charge = mysql_fetch_array($result, MYSQL_ASSOC)) {
//if(mattOnlyTEST()) echo "ZIP: $zip MONTH: $monthYear REV: $rev - ".print_r($charge, 1)."<br>";		
		$chargeCount += 1;
		$monthYear = substr($charge['issuedate'], 0, 7);
		$client = $charge['clientptr'];
		$zip = $zips[$charge['clientptr']];
		$untaxedPaidRev = max(0, $charge['paid'] - $charge['billabletax']);
...
//if(mattOnlyTEST()) echo "ZIP: $zip MONTH: $monthYear REV: $rev<br>";		
		$revenues[$monthYear][$zip] += $rev;
		$actualRevenues[$monthYear][$zip] += $untaxedPaidRev;
...
if(mattOnlyTEST())		echo "CHARGECOUNT (ZIP): $chargeCount";
}
...
//if(mattOnlyTEST())		echo "CHARGECOUNT (SERVICE): $chargeCount";
}

function convertRevenueAndCommissionsForApptsByClient() { //
	global $revenues, $commissions, $taxes, $taxableRevenues;
	...
	foreach((array)$taxableRevenues as $date => $clients)
		foreach($clients as $client => $providers)
			foreach((array)$providers as $provider => $rev)
				$reorg_taxablerevenues[$client] += $rev;
	$taxableRevenues = $reorg_taxablerevenues;
...
function revenueAndCommissionsForApptsByDateClientAndProvider($sql, $omitRevenues, $includeTaxes=true) {
	...
	global $revenues, $commissions, $actualRevenues, $totalActualRevenue, $totalRevenue, $taxableRevenues, $totalTaxableRevenue, $totalCommission, $taxes, $actualTaxes, $clientTaxRates,
  ...
  while($appt = mysql_fetch_array($result, MYSQL_ASSOC)) {
		...
		if(!$omitRevenues) {
			...
			if($includeTaxes) {
				$thisTax = figureTaxForAppointmentInSet($appt, $clientTaxRates);
				if($thisTax != 0 && $thisTax != 0.0) {
					$totalTaxableRevenue += $rev;
//if(mattOnlyTEST() && $client == 1085) echo "<ht>$thisTax<hr>";				
					$taxableRevenues[substr($appt['date'], 0, 7)][$client][$provider] += $rev;
				}
				$taxes[substr($appt['date'], 0, 7)][$client][$provider] += $thisTax;
			}
		}
	}
}
...
function commissionsAndRevenuesByMonthCSV($start, $end, $subCategory, $addRefundSubcat=true, $emptyLabel=null, $sortSubCategories=false) {
  $rows = commissionsAndRevenuesByMonthData($start, $end, $subCategory, $addRefundSubcat, $emptyLabel, $sortSubCategories);
	$columns = explodePairsLine('month|Month||category|Service Type||count|Count||actualRev|Actual Revenue||revenue|Projected Revenue||commission|Commission||net|Projected Net');
	if(0 && !mattOnlyTEST()) unset($columns['count']);
	dumpCSVRow($columns);
	$month = 'All Months';
...
function commissionsAndRevenuesByMonthData($start, $end, $subCategory, $addRefundSubcat=true, $emptyLabel=null, $sortSubCategories=false) {
	...
	$refundTotal = 0;
	foreach((array)$refunds as $monthYear => $refundArray) $refundTotal += array_sum($refundArray);
	$row = array('category'=>$rowCat,
								'count'=>'--',
								'actualRev' => $totalActualRevenue+$refundTotal, 
								'revenue' => $totalRevenue+$refundTotal, 
...
//if(mattOnlyTEST()) {print_r (array_values($revenues));exit;}
	ksort($revenues);
	foreach($revenues as $monthYear => $revenue) {
		$totalMonthServiceCount = 0;
		foreach((array)$visitCounts as $service => $months)
			$totalMonthServiceCount += $months[$monthYear];
		...
		$row['category'] .= " (count: $totalMonthServiceCount)";
		$row['count'] = $totalMonthServiceCount;
...
if(FALSE && !staffOnlyTEST()) {
		...
		foreach($revenue as $service => $amount) {
			...
			$row['count'] = $visitCounts[$service][$monthYear];
			...
			$row['#ROW_EXTRAS#'] = "class='reportdetail'";
			$rows[] = $row;
		}
	}
	return $rows;
...
function commissionsAndRevenuesByClientTable($start, $end, $showCommissions=false) {
	...
	$columns = explodePairsLine('client|Client||lname|Last Name||fname|First Name||services|Services||revenue|Revenue');
...
function commissionsAndRevenuesByClientCSVRows($start, $end, $showCommissions=false, $showClientDetails=false, $omitServiceCounts=false) {
	$data = commissionsAndRevenuesByClientTableData($start, $end, $showCommissions);
	$columns = explodePairsLine('client|Client||lname|Last Name||fname|First Name||services|Services||revenue|Revenue');
	//$columns = explodePairsLine('client|Client||services|Services||revenue|Revenue');
	if($showCommissions) $columns['commission'] = 'Commission';
	if($omitServiceCounts) unset($columns['services']);
	...
	dumpCSVRow($columns);
	// dumpCSVRow($data['totals'], array_keys($columns)); // does not work.  why?
...
function commissionsAndRevenuesByClientDetailedCSVRows($start, $end, $showCommissions=false, $showClientDetails=false) {
	//for the stated period collected revs and commissions and counts per service type/surchargetype/misc charge
	// NO PROJECTIONS
	$clientRevenueDetails = detailedRevenuesAndCommissionsByClient($start, $end);
	$sql = 
		"SELECT clientid FROM tblclient 
		 WHERE clientid IN (".join(',', array_keys($clientRevenueDetails)).")
		 ORDER BY lname, fname";
	logLongError($sql);
	$clientids = fetchCol0($sql);
	$columns = explodePairsLine('clientid|Client ID||fname|Client First||lname|Client Last||label|Service||count|Count||revenue|Revenue');
	if($showCommissions) $columns['commission'] = 'Commission';
	dumpCSVRow($columns);
	foreach($clientids as $clientid) {
		foreach($clientRevenueDetails[$clientid] as $clientservice) {
			$row = array();
			foreach(array_keys($columns) as $col) $row[$col] = $clientservice[$col];
			if($showCommissions) $row['commission'] = $clientservice['commission'];
			dumpCSVRow($row, array_keys($columns));
		}
	}
}
...
function commissionsAndRevenuesByClientTableData($start, $end, $showCommissions=false) {
	...
	$clientNames = fetchAssociationsKeyedBy("SELECT clientid, CONCAT_WS(' ', fname, lname) as client, fname, lname FROM tblclient", 'clientid');
	...
	foreach($clientOrder as $client) {
		...
		$row = $clientNames[$client];
		//$row = array('clientid' =>$client, 'client' =>$clientArr[$client]);
...
function taxesCollectedByClientTable($dataByClient) {
	$columns = explodePairsLine('client|Client||items|Services||payments|Payments||revenue|Revenue||tax|Tax Collected||unknown|Unknown'); // ||total|Total||taxrate|Tax Rate
	$colClasses = array('revenue' => 'dollaramountcell', 'payments' => 'dollaramountcell', 'tax' => 'dollaramountcell', 'unknown' => 'dollaramountcell'); 
	$headerClass = array('revenue' => 'dollaramountheader', 'payments' => 'dollaramountheader', 'tax' =>'dollaramountheader', 'unknown' =>'dollaramountheader' ); //'dollaramountheader'
	foreach($dataByClient as $clientptr => $row) {
		$totalPayments += $row['total'];
		$totalRevenue += $row['revenue'];
		$taxes += $row['tax'];
		$clientVisits += $row['items'];
	}
	foreach($dataByClient as $i => $row) {
		foreach(array('revenue', 'tax', 'total', 'unknown') as $key) {
			$dataByClient[$i][$key] = $row[$key] ? dollarAmount($row[$key]) : '';
			if($key == 'unknown' && $dataByClient[$i][$key] == dollarAmount(0)) // kludge
				$dataByClient[$i][$key] = '';
		}
		if($dataByClient[$i]['payments'])
			$dataByClient[$i]['payments'] = "(".$dataByClient[$i]['payments'].") ".$dataByClient[$i]['total'];
		$dataByClient[$i]['taxrate'] = number_format($dataByClient[$i]['taxrate']*100, 4).' %';
	}
	$clientNames = fetchKeyValuePairs(
		"SELECT clientid, CONCAT_WS(' ', fname, lname) as name 
			FROM tblclient
			WHERE clientid IN (".join(',', array_keys($dataByClient)).")
			ORDER BY lname, fname", 1);
	$clientNames[-999] = "<b>Total</b>";
...
function taxesByClientTable($start, $end, $showCommissions=false, $csv=false) {
	...
	$columns = explodePairsLine('client|Client||services|Services||revenue|Revenue||taxablerevenue|Taxable Revenue||taxrate|Tax Rate||tax|Tax');
	//if(!mattOnlyTEST() && !dbTEST('tonkapetsitters')) unset($columns['taxablerevenue']);
	if($showCommissions) $columns['commission'] = 'Commission';
	$colClasses = array('revenue' => 'dollaramountcell', 'taxablerevenue' => 'dollaramountcell', 'commission' => 'dollaramountcell', 'tax' => 'dollaramountcell'); 
	$headerClass = array('revenue' => 'dollaramountheader', 'taxablerevenue' => 'dollaramountheader', 'commission' => 'dollaramountheader'); //'dollaramountheader'
	
	foreach($rows as $i => $row) {
		$totalRevenue += $row['revenue'];
		$totalTaxableRevenue += $row['taxablerevenue'];
		$totalCommission += $row['commission'];
		$clientVisits += $row['services'];
		$taxes += $row['tax'];
	}
	
	$dollarFields = array('revenue', 'tax');
	if($columns['taxablerevenue']) $dollarFields[] = 'taxablerevenue';
	foreach($rows as $i => $row) {
		foreach($dollarFields as $key) {
...
function taxesByClientData($start, $end, $showCommissions=false) {
	global $refunds, $revenues, $commissions, $actualRevenues, $totalRevenue, $totalCommission, $totalActualRevenue, $clientVisits, 
					$taxes, $taxRatesByClient, $baseRate, $taxableRevenues, $totalTaxableRevenue;
	//$providerNames = getProviderNames();
	//$providerNames[0] = 'Unassigned';
	$clientNames = fetchKeyValuePairs("SELECT clientid, CONCAT_WS(' ', fname, lname) as clientname FROM tblclient", 'clientid');
  ...
  if(!$showCommissions) unset($row['commission']);							 
	...
	foreach($clientOrder as $client) {
		...
		$row['taxablerevenue'] = $taxableRevenues[$client];
		$row['taxrate'] = $taxRatesByClient[$client] ? $taxRatesByClient[$client].' %' : $baseRate.' %';
...
function commissionsAndRevenuesByServiceCSV($start, $end) {
	$rows = commissionsAndRevenuesByServiceData($start, $end); //||count|Count
	$columns = explodePairsLine('category|Service Type||count|Count||month|Month||actualRev|Actual Revenue||revenue|Projected Revenue||commission|Commission||net|Projected Net');
	dumpCSVRow($columns);
	$month = 'All Months';
	foreach($rows as $i => $row) {
		if($row['#PRIMARY#']) {
...
function commissionsAndRevenuesByServiceData($start, $end) {
	...
	$refundTotal = 0;
	foreach((array)$refunds as $whatever => $refundArray) $refundTotal += array_sum($refundArray);
	$row = array('category' => $rowCat,
								'count'=>'--',
								'actualRev' => $totalActualRevenue+$refundTotal, 
								'revenue' => $totalRevenue+$refundTotal, 
...
if(FALSE && !staffOnlyTEST()) {	
	// Refunds Row
	$totalRefunds = 0;
	if($refunds) foreach((array)$refunds as $amounts) {
		$totalRefunds += array_sum($amounts);
		$totalRefundCount += count($amounts);
	}
	$row = array('category' => 'Refunds', 'count'=>$totalRefundCount);
	...
	foreach((array)$refunds as $monthYear => $monthRefunds) {
		// Month Summary
		$row = array('category' => date('F Y', strtotime("$monthYear-01")), 'count'=>count($monthRefunds));
	...
	foreach($revenues as $service => $revenue) {
		...
		$row['count'] = array_sum($visitCounts[$service]);
...
function commissionsAndRevenuesByZIPTable($start, $end, $emptyLabel='No ZIP Code supplied', $csv=false) {
	...
	foreach($rows as $i => $row) {
		...
		if(mattOnlyTEST()) $rows[$i]['category'] = "<span title='[{$row['category']}]'>{$row['category']}</span>";
	}
	//if(mattOnlyTEST()) print_r($rows);
	tableFrom($columns, $rows, 'width=100%', $class=null, $headerClass, $headerRowClass=null, $dataCellClass='sortableListCell', $columnSorts=null, $rowClasses=null, $colClasses, $sortClickAction=null);
}
...
if(FALSE && !staffOnlyTEST()) {
	// Refunds Row
	$totalRefunds = 0;
	if($refunds) foreach((array)$refunds as $amounts) $totalRefunds += array_sum($amounts);
	$row = array('category' => 'Refunds');
	$row['actualRev'] = $totalRefunds;
	$row['revenue'] = $totalRefunds;
	$row['#ROW_EXTRAS#'] = "style='background:lightblue;'";
...
//echo "$secondaryKey: ".print_r($actualRevenues[$secondaryKey],1).'<br>';	
		$refund = 0;
		foreach((array)$refunds as $monthYear => $allZipRefunds)
...
//echo "$secondaryKey: ".print_r($actualRevenues[$secondaryKey],1).'<br>';	
		$refund = 0;
		foreach((array)$refunds as $monthYear => $allCityRefunds)
