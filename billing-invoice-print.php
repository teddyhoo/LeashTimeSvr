...
//displayBillingInvoice($invoiceOrClientId, $firstDay, $lookahead, $firstInvoicePrinted=true, $literal=false, $showOnlyCountableItems=false)
	displayBillingInvoice($id, $firstDay, $lookahead, $first, $literal, $showOnlyCountableItems, $payNow=!'important', $_REQUEST['packageptr']);
	$first = false;
}
?>
...
function printInvoices() {
	...
	ajaxGetAndCallWith('billing-invoices-printed-ajax.php?ids=<?= $idsString."&firstDay=$firstDay&lookahead=$lookahead&literal=$literal" ?>', updateOpener, '<?= $idsString ?>')
}
