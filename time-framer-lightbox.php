<?
/*
* time-framer-lightbox.php - a replacement for time-framer-mouse.php
...
function makeSimplifiedTimeFramer($id, $clientList=TRUE) {
  ...
  echo "<div id='INVISIBOX'  style='display:none;'><div id='PLACEHOLDER$id' class='' style='$width;font-size:1.1em;text-align:center'><form id='timeframer' name='timeframer'>\n<input type=hidden name=originaldate id=originaldate>"; //
  hiddenElement("PLACEHOLDER$id"."_simple", 1);
	$links = array();
  ...
  /*foreach($timeframes as $label => $time) {
	...
	echo "<br><div id='PLACEHOLDERframetimemeaning'style='text-align:center'>&nbsp;</div>";
  echo "\n</form></div></div>";
}
...
function showTimeFramer(e, elId, offset, anchorToElement) {
  ...
  // set vals in timeframer
	updateTimeFramer(val); // PLACEHOLDER$id "$PLACEHOLDERid"."_simple" 'PLACEHOLDERframetimemeaning'
//alert(document.getElementById(timeFrameId).style.visibility+' '+	document.getElementById(timeFrameId).style.left+" "+document.getElementById(timeFrameId).style.top);	
	...
	var timeframeHTML = $("#INVISIBOX").html(); //$("<div />").append($(".timeFrame").clone()).html();
	timeframeHTML = timeframeHTML.replace(/PLACEHOLDER/g, "");
	lightBoxHTML(timeframeHTML, 160, 200);
}
...
function updateTimeFramer(timeframe) {
	...
	od.value = timeframe;
	if(document.getElementById("PLACEHOLDER"+timeFrameId+'_simple')) return; // BINGO!
