...
if(fetchRow0Col0("SELECT value FROM tblpreference WHERE property = 'homeSafeNotifySitters' LIMIT 1")) {
	...
	$appts = fetchAllAppointmentsForNRPackage($hidden['packageptr'], $request['clientptr']);
	$provids = array();
...
//echo "[".join(', ', $provids)."]";exit;
		...
		$clientLabel = clientLabelForSitters($request['clientptr']);
		foreach($provids as $provptr) {
			notifySitterClientHomeSafe($provptr, $clientLabel, $request['clientptr']);
		}
	}
}
...
if(TRUE) {
	...
	notifyStaff('h', "HOME SAFE reported : $clientLabel", "$clientLabel (@{$request['clientptr']})<p>reported Home Safe at ".shortDateAndTime(time()));
}
