...
// for inclusion in the provider editor services tab
...
$provider['name'] = providerShortName($provider);
$comms = getCommsFor($provider, $filter, false);
...
$useGraphicCommButtons = TRUE /* as of 9/18/2020 */|| /*staffOnlyTEST() || */$_SESSION['preferences']['enableMessageArchiveFeature'] || $_SESSION['preferences']['enableSMS'];
if($useGraphicCommButtons) {
	...
	if($showMessageArchiveButton) {
		echo "$buttonSpacer<div class='fa fa-archive fa-2x' $buttonStyle title='View Archived Email' 
				onclick='document.location.href=\"reports-email-archived.php?providerid=$id\"'></div>";
	}
	if($_SESSION['preferences']['enableOfficeDocuments']) {  // enableClientFilesFeatures enabled 12/6/2018
		echo "$buttonSpacer<div class='fa fa-file fa-2x' $buttonStyle title='View Sitter Documents' 
				onclick='openConsoleWindow(\"sitterdocs\", \"provider-files.php?id=$id\",700,700);'></div>";
	}
