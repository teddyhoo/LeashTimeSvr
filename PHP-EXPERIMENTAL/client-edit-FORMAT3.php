...
if($id && dbTEST('leashtimecustomers') && (staffOnlyTEST())) $breadcrumbs .= " - ".fauxLink('Client Biz Details', "descriptionBox()", 1, "Show client business details.");

if($id /*&& ($_SESSION['preferences']['offerNearbySittersMap'])*/) $breadcrumbs .= " - ".fauxLink('Show Nearby Sitters', "openConsoleWindow(\"nearbysitters\", \"client-provider-map.php?pop=1&id=$id\",800,800)", 1, "Open a map")." <span style='color:red;font-variant:small-caps;'>new</span>";

if($id && (staffOnlyTEST() || $_SESSION['preferences']['offerNearbyClientsMap'])) $breadcrumbs .= " - ".fauxLink('Show Nearby Clients',"openConsoleWindow(\"nearbyclients\", \"client-cluster-map.php?pop=1&id=$id\",800,800)", 1, "Open a map")." <span style='color:red;font-variant:small-caps;'>beta</span>";
...
if($client['clientid']) {
	if(!$readOnlyVisits) {
		...
		echo " ";
			...
			if(staffOnlyTEST()) $rawOptions['STAFF Sitters Who Have Served'] = 'staffProvidersWhoHaveServed';
...
function dumpClientBasicTab($client) {
	...
	foreach($fields as $key => $label) {
		...
		if(in_array($key, array('active', 'prospect'))) {
			...
			echo "<tr><td><label for='fname'>$label:</label></td><td><input class='standardInput' id='fname' name='fname' value='".safeValue($client['fname'])."' onBlur= autocomplete='off'> <div id='dupnames' style='display:inline;color:darkgreen;text-decoration:underline;cursor:pointer;' onclick='alert(\"Similar names:\"+String.fromCharCode(13)+this.title)'></div></td></tr>\n";
		}
	...
	echo "\n<td>";
	electElement('Veterinarian:', 'vetptr', $client['vetptr'], array(), 'vetChanged(this)');  // init'd by rebuildSelectOptions
	...
	if($_SESSION['ccenabled']) {
		...
		if($client['clientid']) {
			...
			//if ($_SESSION['preferences']['ccGateway'] == 'Solveras') {
				//$solverasObj = new SolverasGateway();
				/*if ($solverasObj->achEnabled()){
...
function dumpClientBasicKeyVal($client, $raw) {
	echo "<table width=100%>\n";
	echo "<tr><td valign=top><table width=100%>\n"; // R1C1
	echo "</table></td>\n"; // end R1C1
	echo "<td valign=top><table width=90%>\n"; // R1C2
...
function dumpClientBasicPhone($client, $raw) {
	for($i=0;$i < count($raw) - 1; $i+=2) $fields[$raw[$i]] = $raw[$i+1];
	foreach($fields as $key => $label) {
		$val = isset($client[$key]) ? $client[$key] : '';
		if(in_array($key, array('cellphone','cellphone2','homephone','workphone')))
...
<script language='javascript' src='edit-client.js'></script>
<script language='javascript'>
...
function clickTab(tabid) {
	...
	let pageid = "tabpage_"+pageid;
  	let notByName = false;
  	let els = document.getElementsByName('tabcell');
  	if(els.length == 0) {
		notByName = true;
...
function checkForDups() {
	...
	if(fname && lname.length > 1) {
		   ('possible-duplicate-clients.php?justNames=1&fname='+escape(fname)+'&lname='+escape(lname), showDups, 0)
	}
}
...
function showKeyCopies(sel) {
  ...
  for(var i=1; i<=<?= $maxKeyCopies ?>; i++) {
    var displayMode = i <= num ? 'none' : 'none';
