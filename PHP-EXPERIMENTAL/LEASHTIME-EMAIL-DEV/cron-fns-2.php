...
function runCrons() {
	...
	foreach(fetchAssociations("SELECT * FROM tblpetbiz WHERE activebiz=1 $TEST") as $biz) {
		...
		foreach($dailyTasks as $jobName => $jobTime) {
			...
			//cronSendMasterSchedule();
			cronSendProviderSchedules();
...
function cronSendProviderSchedules() {
	...
	require_once "prov-notification-fns-2.php";
	require_once "preference-fns.php";
...
function cronSendMasterSchedule() {
	require_once "master-schedule-email-fns-2.php";
	require_once "prov-schedule-fns-2.php";
...
function runEmailRelatedJobs() {
	...
	echo ("BIZ ID: "  . $biz['bizid']  . "\n\n");
	if(!function_exists('userRole')) { // needed by prov-schedule-fns.php
		function userRole() {
			echo "returning o status";
			return 'o';
		}
		function adequateRights($needed) {
			echo "adequate rights";
			return true;
		}
	}	else {
		echo "CONTINUE RUNNING EMAIL RELATED JOBS ...\n\n";
	}
	// TEMPORARY -- UNTIL WE SWAP IN THE NEW MAIL ENGINE <== Hah!
	if($hhost = isSMTPHostExperimental()) {
		echo "EXPERIMENTAL SMTP";
	...
	$NO_SESSION['i18n'] = getI18NProperties($biz['country']);		
	echo ('SESSION: ' . $NO_SESSION['I18N'] . '\n\n');

	//$strm = fopen('./logtest/writeCron.txt','a');
	//echo "--------------------GENERATING MESSAGE CONFIRMATIONS --------------------\n";
	/*fwrite($strm, "--------------------GENERATING MESSAGE CONFIRMATIONS --------------------\n");
	$messageConfirmations = enqueueProviderMemos();  // queuedemail['emailid']=>confirmations
	foreach ($messageConfirmations as $msgCon) {
		fwrite($strm,"M: $msgCon");
	}
	fwrite($strm,"$messageConfirmations \n");
	fwrite($strm, "-------------------- END MESSAGE CONFIRMATIONS  --------------------\n\n");

	fwrite($strm, "--------------------INVOICE PREVIEW--------------------\n");
	preProcessInvoicePreviewEmails();
	fwrite($strm, "--------------------END PREVIEW--------------------\n\n");
*/
