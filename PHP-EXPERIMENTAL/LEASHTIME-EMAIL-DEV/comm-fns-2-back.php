<? // comm-fns.php
	require_once "request-fns-2.php";
...
function notifyByLeashTimeSMS($person, $body, $media=null) {
	...
	require_once "field-utils-2.php"; // for phone numbers
...
function registerLatestSMS($phone, $ownertable, $ownerptr, $bizptr) {
	...
	require "common/init_db_common-2.php";
...
function clearEmailQueueIfDisabled() {
	...
	echo "MAIL QUEUE DISABLED VALUE: $mailQueueDisabled\n";
	...
	if($mailQueueDisabled = strtotime($mailQueueDisabled)) { // strtotime returns FALSE if not a date or datetime
		...
		if(time() - $mailQueueDisabled > $deleteAfterHours * 60 * 60) {
			...
			if($count = mysqli_affected_rows()) {
				echo ("$count msgs deleted after at least $deleteAfterHours\n");
				logError("Email queue cleared ($count msgs deleted after at least $deleteAfterHours hours).");
			}
			return true;
		}
	}
...
function enqueueEmailNotification($person, $subject, $body, $cc=null, $mgrname, $html=false, $originator=null, $tags=null, $attachments=null) {
	...
	/*$strm = fopen('./logtest/writeCron.txt','a');
	echo  "ENQUEUE EMAIL NOTIFICATION WITH THE FOLLOWING INFORMATION:\n\n";
	fwrite($strm, "ENQUEUE EMAIL NOTIFICATION WITH THE FOLLOWING INFORMATION:\n\n");
	fwrite($strm, "----------------------------------------------------------------------------------\n");
	fwrite($strm, "$person\n");
	foreach($person as $p) {
		fwrite($strm, "PERSON: $p\n");
		echo "PERSON: $p\n";
	}
	fwrite($strm, "-------SUBJECT-------\n$subject\n");
	fwrite($strm, "-------HTML-------\n$html\n");
	...
	if(clearEmailQueueIfDisabled()) {
		echo "EMAIL QUEUE IS IS DISABLED, AND RETURNING";
		return;  // don't queue up email if queue has been disabled longer than the threshold period
	}
    global $TESTEMAILOVERRIDE;
    if($TESTEMAILOVERRIDE) {
        $person['email'] = $TESTEMAILOVERRIDE;
        echo "TESTMAIL OVERRIDE TRIGGERED";
    }
	
	if(!$person['email']) { 
		echo "No email found for {$person['fname']} {$person['lname']}.";
		return array("No email found for {$person['fname']} {$person['lname']}.");
	}
	...
	$mgrname = $mgrname ? $mgrname : 'System';
	$tblmsgfield = queuedMessageDescription($mgrname, $correspid, $correstable, $originatorid, $originatortable, $tags, $attachments);
	/*echo "TABLE MESSAGE FIELDS\n---------------------------- \n$tblmsgfield\n\n";
	echo ("Table message fields type: " . gettype($tblmsgfield) . "\n");
	fwrite($strm, "----------------------------------------------------------------------------------\n");
	fwrite($strm, "INSERTING FOLLOWING IN THE TBLQUEUEDEMAIL\n");
	fwrite($strm, "----------------------------------------------------------------------------------\n");

	$rc = $person['email'];
	fwrite($strm, "CORRESPID: $correspid\n");
	fwrite ($strm, "ORIGINATOR: $originatorid\n");
	fwrite ($strm, "ORIGINATOR TABLE: $originatortable\n");
	fwrite ($strm, "MANAGER NAME: $mgrname");
	fwrite($strm, "RECIPIENTS: $rc\n");
	fwrite($strm, "SUBJECT: $subject\n");
	fwrite($strm, "BODY: $body\n");
	fwrite($strm, "CC: cc\n");
	fwrite($strm, "ADDED TIME: $addedtime\n");
	fwrite($strm, "HTML: $html\n");
	fwrite($strm, "----------------------------------END------------------------------------------------\n");*/
	$tblmsgfieldsstr = queuedMessageDescription($mgrname, $correspid, $correstable, $originatorid, $originatortable, $tags, $attachments, 1);
	/*fwrite($strm, "$tblmsgfieldsstr\n");
	echo "TABLE MESSAGE FIELD STRING:\n_______________________\n$tblmesgfieldstr\n";
	fclose($strm);*/
...
function clearedLogJamTEST($mailQueueSendStarted) {
	...
	*/
	...
	if(time() - strtotime($mailQueueSendStarted) < 10 * 60) {
		echo "MAIL QUEUE LESS THAN 10 MIN OLD";
		return false;
	}
...
function toggleSupportCronMailPrefs($switchback=false) {  // NEEDS WORK -- NO SESSION
	...
	echo "TOGGLE SUPPORT CRON MAIL PREFS\n";
	if(!$switchback) {
...
function sendAllQueuedEmail() {  // return message ids keyed by queued message ids
	...
	if($mailQueueDisabled) {
		echo "THE MAIL QUEUE IS DISABLED";
	...
	echo "MAIL QUEUE STATUS: $mailQueueSendStarted";
	if($mailQueueSendStarted) {
		echo "MAIL QUEUE SEND STARTED: TRUE";
		...
		echo "MAIL QUEUE SEND STARTED: FALSE";
		//insertTable('tblpreference', array('property'=>'mailQueueSendStarted', 'value'=>date('Y-m-d H:i:s')), 1);
		echo "INSERTED TABLE PREFERENCE MAIL QUEUE SEND STARTED";
	}
	$messages = array();
	$sent = 0;
	$errorCount = 0;
	...
	foreach(fetchAssociations("SELECT * FROM tblqueuedemail ORDER BY emailid") as $i => $queuedmsg) {
		...
		if($quotaReached = emailQuotaExceeded($i)) {
			fwrite($strm,  "Queue halt after LT quota reached ($quotaReached). Msgs sent: $sent.  Errors: $errorCount");
			logError("Queue halt after LT quota reached ($quotaReached). Msgs sent: $sent.  Errors: $errorCount");
			break;
		}
		if($errorCount >= $errorQuota) {
			fwrite($strm,  "Queue halt after $errorQuota errors. Msgs sent: $sent.  Errors: $errorCount");
			logError("Queue halt after $errorQuota errors. Msgs sent: $sent.  Errors: $errorCount");
			break;
		}
		$msgkeys = array_keys($queuedmsg);
		foreach ($msgkeys as $key) {
			echo ("QUEUE msg key ->" . $key . " Value: ".$queuedmsg[$key] + "\n");
			//echo "with type: " . getttype($queuedmsg[$key]);
		}
		fwrite($strm,  "------------------QUEUED MESSAGE -------------------\n");
		fwrite($strm,  "SUBJECT: " . $queuedmsg['subject']);
		fwrite($strm,  "\n");
		fwrite($strm,  "BODY: " . $queuedmsg['body']);
		fwrite($strm,  "\n");
		fwrite($strm,  "---------------------END--------------------------------\n");
...
function sendQueuedEmail($queuedmsg) {
  ...
  foreach(array('recipients'=>'To', 'cc'=>'CC', 'bcc'=>'BCC') as $field => $label)
  	if($queuedmsg[$field])
          $correspaddr[] = "$label:{$queuedmsg[$field]}";
    if($correspaddr)
        $correspaddr = join('|', $correspaddr);
...
function sendEmailIntercept($recipients, $subject, $body, $cc=null, $html=null, $senderLabel='', $bcc=null, $extraHeaders=null, $attachments=null) {
...
function cleanUpAttachments($queuedmsg) {
	...
	foreach($attachments as $att) {
		...
		if(in_array($att['path'], (array)$allFiles)) {
			continue;
		} else {
			unlink($att['path']);
...
function getSMSCommsFor($correspondent, $filter=null, $clientflg=true, $totalMsg=false) {
	...
	foreach($messages as $msg)
		if($msg['originatorid'] && $msg['originatortable'])
			$originators["{$msg['originatortable']}-{$msg['originatorid']}"] = array('originatortable'=>$msg['originatortable'], 'originatorid'=>$msg['originatorid']);
	foreach((array)$originators as $k => $v)
		$originators[$k] = $v['originatortable'] == 'tblclient' ? fetchRow0Col0("SELECT CONCAT_WS(' ', fname, lname) FROM tblclient WHERE clientid = {$v['originatorid']}"): fetchRow0Col0("SELECT ifnull(nickname, CONCAT_WS(' ', fname, lname)) FROM tblprovider WHERE providerid = {$v['originatorid']}");

	require_once "field-utils-2.php";

	foreach($messages as $msg) {
		if($correspondent == -1) {
			if($msg['correstable'] == 'tbluser') 
				$corr = getUserByID($msg['correspid']);
			else {
		...
		else $sender = $msg['inbound'] ? $correspondentName : ($msg['originatorid'] ? $originators["{$msg['originatortable']}-{$msg['originatorid']}"] : $msg['mgrname']);
	...
	if($corresTable != 'tbluser') { // begin request search
		...
		foreach(fetchAssociations($sql) as $req) {
			...
			/*if($clientflg && $req['providerptr']) {
				require_once "provider-fns-2.php";
...
function remindersLink($label, $type, $id) {
	/*require_once "reminder-fns-2.php";
