	...
	if($emailRecipient) {
		...
		if($messageAppendix) {
			if($messageAppendixToken && strpos($message, $messageAppendixToken) !== FALSE) {
				...
				$message .= html_entity_decode($messageAppendix);
			}
		}
    
		if($error = sendEmail($recipients, $subject, $message, null, $htmlMessage, $_POST['mgrname'])) {
            $debugStr = $_POST['mgrname'];
			$error = "userNotify:$error, $debugStr (recipients: ".htmlentities(join(',', $recipients)).")";
        }
		...
		$error = "No email address found for {$recipient['fname']} {$recipient['lname']}.";
