...
function appointmentDescriptionHTML($appointmentid, $package=null, $returnNull=null) {
	...
	if(isset($_SESSION) && $_SESSION) $serviceName = getServiceName($appt['servicecode']);
		...
		require_once "service-fns-2.php";
...
function correctStartAndEndTimes($appt) {
	// 2/17/2021 correction to make starttime/endtime agree with timeofday
	$times = timesForApptStartAndEnd($appt);
	$times['starttime'] = date('H:i:s', $times['start']);
	$times['endtime'] = date('H:i:s', $times['end']);

	if($appt['starttime'] != $times['starttime'] || $appt['endtime'] != $times['endtime']) {
		$mods['starttime'] = $times['starttime'];
		$mods['endtime'] = $times['endtime'];
		updateTable('tblappointment', $mods, "appointmentid = {$appt['appointmentid']}", 1);
		$note = 'Fixed starttime and endtime.';
		logChange($appt['appointmentid'], 'tblappointment', 'm', $note);
		return "$note in {$appt['appointmentid']}";
	}
}
  ...
  $today = strtotime(date('Y-m-d'));
	...
	for($day = $start; $day <= $end; $day = strtotime("+ 1 day", $day)) {
		...
		foreach($services as $service) {
			if(dbTEST('dogslife') && $package['clientptr'] == 1999) 
...
function changeAppointmentDate($appointmentid, $newDate) {
	...
	if($oldAppt['recurringpackage'] && $oldAppt['serviceptr']) 
...
function changeRecurringAppointmentDate($appointmentid, $newDate) {
	...
	// ###################################################################################################
	if(FALSE && dbTEST('dogslife') && $package['clientptr'] == 47) 
		bagText(date('H:i')." [".print_r(date('Y-m-d', $start), 1)." - ".date('Y-m-d', $end)
							."] Week: ($weekNumber) Day: ".print_r(date('Y-m-d', $day), 1)
							." Svc: ".print_r($service, 1), 'MULTITEST');
...
//print_r($appt);echo "<hr>";
	$appt = createAppointment('recurring', $package, $appt, strtotime($appt['date']), $canceledBecause=null, $simulation=false);
//print_r($appt);exit;
	$appt['custom'] = 1;
...
//function labelRow($label, $name, $value=null, $labelClass=null, $inputClass=null, $rowId=null,  $rowStyle=null, $rawValue=false)
  ...
  $serviceSelections = array_merge(array(''=>''), getServiceSelections());
	...
	if(!adequateRights('#pa')) { // dbTEST('tonkatest') &&  removed 4/20/2021
		hiddenElement('rate', $source['rate']);
		echo "<div id='div_rate' style='display:none'>{$source['rate']}</div>";
		hiddenElement('bonus', $source['bonus']);
	}
	else {
...
//function labelRow($label, $name, $value=null, $labelClass=null, $inputClass=null, $rowId=null,  $rowStyle=null, $rawValue=false) {
	...
	if($_SESSION['preferences']['sittersPaidHourly']) {
		...
		labelRow($apptFields['rateline'].':', '', 
 		...
 		if($showClientCharge && $source['surchargenote']) labelRow($apptFields['surchargenote'].':', '', $source['surchargenote']);
	}
...
function cancelAppointments($ids, $cancel, $additionalMods=null, $generateMemo=true, $initiator=null) { // return affected providers as a csv string
	require_once "invoice-fns-2.php";
	//require_once "invoice-fns.php";
