    ...
    function merchantInfoLabels() {
		...
		function executeTransaction($auth, $cc, $transactionType, $x_amount, $otherData=null) {
			...
			if($specificTransaction == 'ACHSale') 
				...
				global $db;
				require_once "log-cc.php";
				requestWrite('TRANSFIRST TXP-GATEWAY-CLASS EXECUTE TRANSACTION');
				if($db == 'pamperedpetspa' || $db == 'leashtimecustomers') {
					// create a "masked" request
					$maskedRequest = $cc;
					...
					// log the request to see if the calculated amount is correct
...
//echo "<p>transactionSource: $transactionSource<hr>transactionType: $transactionType<hr>specificTransaction: $specificTransaction<hr>postRequest: ".print_r($postRequest, 1)."<br>gatewayURL: ".print_r($gatewayURL, 1);
//if(mattOnlyTEST() && $cc['ccid'] = 949) {print_r($postRequest);exit;}
			...
			else if($responseCodeFound && $response['ResponseCode'] == '00' && trim("{$response['tranNr']}")) return $response['tranNr'];
		...
		function createTXPCCSalePostRequest($auth, $account, $amount) { // TXP
			...
			$pairs = array(
				...
				'IndustryCode' => 2, //  0 = Direct Marketing / MOTO, 1 = Retail, 2 = eCommerce, 3 = Mobile Payment * Required for card present Master Card Transactions
				'AccountNumber' => $account['x_card_num'],
				'CVV2' => $account['x_card_code'],
				'ExpirationDate' => date('ym', strtotime($account['x_exp_date'])),
				'Amount' => $formattedAmount, // translated into 'minor denomination' // DOES NOT WORK RELIABLY: (int)($amount * 100), intval($amount * 100)
				//'TaxIndicator' => ,???
				//'TaxAmount' => ,
				// 'PONumber' => ???
				// 'CustRefID' => , clientid?
...
//logError("BOING! [{$response['ResponseCode']}]".print_r("[$message]",1));			
