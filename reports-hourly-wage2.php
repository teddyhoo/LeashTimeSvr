...
function fetchWages($start, $end, $providerid) {
  ...
  while($appt = mysql_fetch_array($result, MYSQL_ASSOC)) {
		...
		$appt['hours'] = $hours;
		$appt['pay'] = $appt['rate']+$appt['bonus']; //$hours * (float)$allRates[$prov];
	...
	$sql = "SELECT amount as pay, issuedate as date, 'Gratuity' as service, providerptr,
					CONCAT_WS(' ', tblclient.fname, tblclient.lname) as client
					FROM tblgratuity
					LEFT JOIN tblclient ON clientid = clientptr
...
function wagesTable($start, $end, $rows) {
	global $allVisitTotals, $allowanceTotals, $provs, $allRates, $allTravel, $allSurchargeTotals, $allGratuityTotals;
...
//echo print_r($rows[$i], 1)."<br>";		
		...
		if($rows[$i]['providerptr'] != $prov) {
			...
			$finalrows[] = array('#CUSTOM_ROW#'=>"<tr class='topline'><td colspan=$numCols class='fontSize1_1em'><b>{$provs[$prov]}</b> Rate: $rate  Travel Allowance: $allow </td></tr>");
