...
$pageTitle = "Revenue Report by ZIP Code";
...
<?
	radioButtonRow('Organized:', 'reportType', ($reportType ? $reportType : 'monthly'), array('by month'=>'monthly', 'by ZIP'=>'zip'), $onClick=null, $labelClass=null, $inputClass=null, $rowId=null,  $rowStyle=null);
?>
	</table>
<?
...
<?
	...
	header("Content-Disposition: inline; filename=Client-Revenue-by-ZIP.csv ");
	dumpCSVRow('Revenue Report by ZIP Code');
	dumpCSVRow("Report generated: ".shortDateAndTime('now', 'mil'));
	dumpCSVRow("Period: $start - $end");
}
else {
	$windowTitle = 'Revenue Report by ZIP Code';
	require "frame-bannerless.php";
	fauxLink("<h1 style='display:inline;'>Click Here to Print</h1>", "printThisPage(this);");
	echo "<h2 style='text-align:center'>Revenue Report by ZIP Code</h2>";
...
if($start && $end) {
	...
	revenuesAndCommissionsByZIP($start, $end);
	dropProjectionApptTable();
	dropProjectionSurchTable();
//	refunds($start, $end, 'byZips');
//print_r($refunds);	
	if($reportType == 'monthly') commissionsAndRevenuesByMonthTable($start, $end, "ZIP Codes", false, 'No ZIP Code Supplied', true, $csv);
	else if($reportType == 'zip') commissionsAndRevenuesByZIPTable($start, $end, 'No ZIP Code Supplied', $csv);
}
if(!$print && !$csv){
