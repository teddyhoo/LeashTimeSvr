...
function notifyStaffOfClientRequest($request) {
	...
	if($request['requesttype'] == 'Spam') {
		...
		$msgBody .= "<p>".count($changekeys)." fields changed: (".join(', ', $changekeys).").";
	}
	...
	else if(in_array($request['requesttype'], array('SystemNotification'))) {
		$subject = $request['subject'];
		$requestLink = "<a href='$urlBase'>$subject</a>";
	...
	if($request['scope']) {
		if(strpos($request['scope'], 'day_') === 0)
	...
	foreach($additionalFields as $key => $label)
		if($request[$key]) $msgBody .= "<p>$label:<br>{$request[$key]}";
...
//if(mattOnlyTest()) logError("[SPAM: $spam] notifyStaff($eventType, $subject,...)");	
	if(!$spam) notifyStaff($eventType, $subject, $msgBody);
}
		...
		foreach($extraFields as $key => $value) {
			...
			if(strpos($label, 'meetingdate') === 0 || strpos($label, 'meetingtime') === 0) {
				...
				if($fld == 'referralcode') {
					...
					$value = join(' > ', getReferralPath($value));
				}
...
function saveNewSystemNotificationRequest($subject, $note, $extraFields = null) {
	$request = array();
...
function saveNewClientRequest($request, $notify=true) {
	$requestFields = getRequestFields(); // to avoid problem where globals are nulled on repeated loadings
	if(!isset($request['resolved'])) $request['resolved'] = 0;
...
function saveNewProspectRequest($request) {
	...
	if($request['requesttype'] == 'Spam') {
		if($prefs['autoDeleteSpam']) {
			logChange(99, 'spam', 'd', "IP[{$_SERVER['REMOTE_ADDR']}] spam intercepted");
			return;
		}
...
function requestAssignmentHistory($reqid) {
	...
	foreach($history as $item) 
		$rows[] = 
			...
			'assignedto'=>$ownerNames[$item['note']], 'assignedby'=>$ownerNames[$item['user']]);
	return $rows;
}
...
function separatePreferredSection(&$requests) {
	...
	$PreferredRequestType = mattOnlyTEST() ? 'Reminder' : null;
...
//tableFrom($columns, $data=null, $attributes=null, $class=null, $headerClass='sortableListHeader', $headerRowClass=null, $dataCellClass='sortableListCell', $columnSorts=null, $rowClasses=null, $colClasses=null, $sortClickAction=null) {
	...
	// Flexibility Tweak
	if(staffOnlyTEST() && $customColumns = fetchRow0Col0("SELECT value FROM tblpreference WHERE property = 'requestlistcolumns' LIMIT 1", 1)/*$_SESSION['preferences']['requestlistcolumns']*/) {
