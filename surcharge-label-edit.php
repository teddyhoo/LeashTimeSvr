...
locked('o-');
$id = $_REQUEST['id'];
$currentLabel = fetchRow0Col0("SELECT label FROM tblsurchargetype where surchargetypeid = $id");
if($_POST['action'] == 'change') {
	updateTable('tblsurchargetype', array('label'=>$_POST['typelabel']), "surchargetypeid = $id", 1);
	$_SESSION['frame_message'] = 'Surcharge label changed.';
	echo "<script language='javascript'>parent.location.href = 'surcharge-types.php';</script>";
	exit;
}

$allSurchargeTypeLabels = fetchKeyValuePairs("SELECT surchargetypeid, UPPER(label) FROM tblsurchargetype");
$extraBodyStyle = 'background-image:url("");font-size:1.05em;';
require_once "frame-bannerless.php";
?>
You are editing the label for surcharge type #<b><?= $id ?></b>, which is currently labeled
...
function checkAndSubmit() {
	...
	var duplicate = '';
	<? foreach($allSurchargeTypeLabels as $typeId => $ucaseLabel)
	...
	//alert(ucaseLabels[i]+": "+ucaseLabels[i+1]);
		if(ucaseLabels[i] != <?= $id ?> && ucaseLabels[i+1] == ucaseTypelabel)
			duplicate = "There is already another surcharge type with this name.";
