...
function postSurchargeChange() {
	...
	//echo "OLDCANCELCOMP: [$oldCancelcomp]  CANCELCOMP:  [$cancelcomp]";exit;
		if($oldCancelcomp && !$cancelcomp) {
			...
			$amount = ($rate ? $rate : 0);
			insertTable('tblothercomp', array('appointmentptr'=>$appointmentptr, 'amount'=>$amount, 'comptype'=>'cancelsurcharge', 
	...
	if(!$error) {
		...
		if(dbTEST('dnipetcare')) {
			echo "<h2>Changes saved</s>Please click ";
			fauxLink("here", "if(parent.update) parent.update('appointments', '$postReturn');");
			echo " to continue.";
		}
		else {
...
if($objtype == 'visit') {
	...
	if($id && staffOnlyTEST()) {echo "<a href='appt-analysis.php?id=$id' target=analysis>Analyze</a> - ";}
