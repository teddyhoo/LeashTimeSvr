...
<?
	foreach($vets as $vet) {
		...
		$clinic = $vet['clinicid'] ? "&clinic=1" : '';
		if($clinic)
			$editVetAction = "document.location.href=\"viewVet-mobile.php?id={$vet['clinicid']}$clinic\"";
		else
			$editVetAction = "document.location.href=\"viewVet-mobile.php?id={$vet['vetid']}$clinic\"";
...
function getVetsWithAddresses($lat, $lon, $miles, $address=null) {
	...
	$q = "SELECT street1, street2, city, state, zip, XXXid, clinicname, officephone, cellphone, pager, homephone
				FROM tblXXX
				WHERE LENGTH( CONCAT_WS( '', street1, city, state, zip ) ) >0 AND 
	...
	foreach(fetchAssociations(str_replace('clinicname', "CONCAT_WS(' ', fname, lname) as clinicname", 
															str_replace('XXX', 'vet', $q))) as $clinic)
		$list[] = $clinic;
	foreach($list as $i => $clinic) {
		$list[$i]['address'] = $clinic['address'] = oneLineAddress($clinic);
		$list[$i]['distance'] = getDistance($clinic['address'], $lat, $lon, $miles);
//echo "<font color=red>{$list[$i]['distance']}</font> {$clinic['address']}<br>";
		foreach($phones as $fld) if($phone = $clinic[$fld]) break;
		$list[$i]['phone'] = $phone;
	}
	usort($list, 'cmpDistance');
	return $list;
}

function getDistance($toAddress, $latitude, $longitude, $miles=true) {
	$coords = getLatLon($toAddress);
//echo "<font color=blue>".$googleMapAPIKey."</font><p>";	
	if(!$coords) {
//echo "<font color=red>[[$toAddress - ".print_r($map->geoGetCoords($toAddress), 1)."]]</font><p>";
  ...
  $theta = $longitude - $thisLon; 
  $dist = sin(deg2rad($latitude)) * sin(deg2rad($thisLat)) +  cos(deg2rad($latitude)) * cos(deg2rad($thisLat)) * cos(deg2rad($theta)); 
  $dist = acos($dist); 
  $dist = rad2deg($dist); 
  $milesDistance = $dist * 60 * 1.1515;
//echo "VET: $latitude, $longitude HERE: $thisLat, $thisLon <b>[$milesDistance]</b><p>";
	return round(10 * ($miles ? $milesDistance : $milesDistance * 1.609344)) / 10;
}
...
if($lat) {
	...
	$showLat = 'inline';
	$url = "mobile-nearby-vets.php?go=1&lat=$lat&lon=$lon";
}
else $showLat = 'none';
?>
...
<?
//echo getQRCodeFileName('hello');
//require_once "common/init_db_common.php";
//echo getQRCode('https://leashtime.com/mobile-nearby-vets.php', 'force');
...
function setLatLon(lat, lon) {
	...
	document.getElementById('latlon').href = "mobile-nearby-vets.php?go=1&lat="+lat+"&lon="+lon;
	document.getElementById('latlonspan').style.display = 'inline';
}
function setZipCode(lat, lon) {
	ajaxGetAndCallWith('mobile-nearby-vets.php?lat='+lat+'&lon='+lon, 
