...
if($_REQUEST['separateJavascript']) 
	$sourceHeadContent = <<<SOURCEHEADER

<!-- Javascript for popup calendars, constraint checking, etc. -->
<script language='javascript'>var pathHome = "./";</script>
<script language='javascript' src='https://{$_SERVER["HTTP_HOST"]}/popcalendar.js'></script>
<script language='javascript' src='https://{$_SERVER["HTTP_HOST"]}/check-form.js'></script>
<!-- The following script source should be replaced with your own hosted (and possibly customized) version -->
<script language='javascript' src='prospect-embeddable.php?bizid=$bizid&justJavascript=1'></script>

SOURCEHEADER;
else {
	$headerCommentEnd = strpos($SEPARATE_JAVASCRIPT_FILE_CONTENT, '*/')+2;
	$headlessJavascript = substr($SEPARATE_JAVASCRIPT_FILE_CONTENT, $headerCommentEnd);
	$embeddedJavascript = <<<EMBEDDEDJAVASCRIPT
<script language='javascript'>var pathHome = "./";</script>
<script language='javascript' src='https://{$_SERVER["HTTP_HOST"]}/popcalendar.js'></script>
<script language='javascript' src='https://{$_SERVER["HTTP_HOST"]}/check-form.js'></script>
<script language='javascript'>$headlessJavascript
</script>

EMBEDDEDJAVASCRIPT;
}
if((staffOnlyTEST() || userRole() == 'z') && $_REQUEST['preview'] == 1) {
	$staffONLYNote = <<<staffONLYNote
<div style='background:lightgrey;'><font color=red>STAFF NOTE:<p></font>
See ADMIN > Preferences > LeashTime Staff Only > Prospect Request Response.<p>
This can be<ul><li>empty -- a no-op<li>a URL (same as using the goback input field in this form)<li>not a URL, 
in which case it is displayed as a message in the LeashTime client frame (page: prospect-request-thankyou.php).<br>#HOMEPAGELINK# can be included 
in the message to link back to business Home page.
</ul>
</div>
staffONLYNote;
}
