...
//calendarSet($label, $name, $value=null, $labelClass=null, $inputClass=null, $includeArrowWidgets=true)
	...
	<tr>
	<td valign=top style='padding:0px;padding-botton:2px;margin:0px;margin-botton:2px;'><div id='appointments'  style='border:solid black 1px;height:500px;overflow:auto;'></div></td>
	<td valign=top style='padding-top:0px;'><div id='allappointments' style='border:solid black 1px;height:500px;overflow:auto;'></div></td>
	</tr>
...
//echo $sql.'<p>'.print_r($appts, 1);exit;
  ...
  echo "\n<span class='taskTableProviderCell' colspan=1>Reassign jobs from: ".
         echoProviderSelect('allproviders', $providerptr, $additions, $exclude)." $wagButton</span></p>";/*." on ".date("M d",strtotime($date))*/
  if($providerptr) {
    ...
    echo "<span colspan=3 style='text-align:center;'>$label $dateRangeLabel</span><p>\n";
    $lastDate = '';
    if(empty($appts)) echo "<span colspan=3 style='text-align:center;font-style:italic;'>None</span><p>\n";
    else foreach($appts as $appt) {
			if($appt['date'] != $lastDate) echo "<div class='daybanner'>".date('F j', strtotime($appt['date']))."</div>";
			$lastDate = $appt['date'];
...
function getProvisionalAppointmentsFor($prov, $dateRange, $exclude=null) {
  ...
  echo "\n<span class='taskTableProviderCell' colspan=3>Reassign jobs to: ".
...
//if($_SESSION['staffuser']) {echo "start: $start end: $end ";print_r($timeoff);}
    ...
    echo "<span style='font-size:1.2em'>$label $dateRangeLabel</span>$timeoff<p>\n";         
    $lastDate = '';
    if(empty($appts)) echo "<span style='text-align:center;font-style:italic;'>None</span>\n";
    else foreach($appts as $appt) {
...
function getReassignmentsFrom($prov, $dateRange) {
  ...
  if($prov) {          
    echo "\n<span class='taskTableProviderCell' colspan=3>$pLabel</span><p>";
    $assignee = null;
    $lastDate = '';
    if(empty($appts)) echo "None";
    else foreach($appts as $appt) {
		  if($appt['assignee'] != $assignee) {
				$assigneeName = lookupProviderName($appt['assignee']);
			  echo "<span colspan=3 style='font-style:italic;'>Assigned to $assigneeName</span><p>\n";
...
function reassignAppointmentTo($apptid, $prov, $origprov) {
	$appt = fetchFirstAssoc("SELECT appointmentid, date, timeofday, providerptr FROM tblappointment  WHERE appointmentid = $apptid LIMIT 1");
	require_once "provider-fns.php";
//echo "providerIsOff($prov, {$appt['date']}, {$appt['timeofday']}): ".providerIsOff($prov, $appt['date'], $appt['timeofday']);
	
	if(providerIsOff($prov, $appt['date'], $appt['timeofday']))
		return;
...
function displayAppointmentTile($appt, $prov=null, $zone, $provisionalButNotMovable=false) {
  ...
  $originalOwner = lookupProviderName($appt['providerptr']); 
	...
	$divClass = $movable ? 'dragappt' : ''; 
	echo "<div class='$divClass' $style id='dap_$apptid'><table style='border:outset #888855 1px;width:99%;;margin:3px;'><tr>
