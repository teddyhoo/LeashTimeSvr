<? // sms-prov-client.php
// gated on enableSitterToClientSMS
// for  sitter use
// parameter appt must be to one of sitter's clients
...
locked('p-');
$continue = $_SESSION["preferences"]['enableSMS'] && $_SESSION['preferences']['enableClientSMS'];
$continue = $continue && $_SESSION['preferences']['enableSitterToClientSMS'];
...
if($continue) {
	if($apptid) {
		...
		$targetClient = $appt['clientptr'];
		$clientList = getActiveClientIdsForProvider($_SESSION["providerid"], $includeClientsWithoutAppointments=false, $daysAhead=null, $daysBehind=null);
		if(!in_array($targetClient, $clientList)) {
			$error = "Sorry, this client cannot be texted.";
		}
		else if(!($correspondent = getClient($targetClient))) {
			$error = "Sorry, client not found.";
		}
		else if(!($primarySMSPhoneNumber = findSMSPhoneNumberFor($correspondent))) {
			$error = "Sorry, this client is not textable.";
		}

		$label = 'Client';
		$arg = "appt=$apptid";
	}
	else $error = "Sorry, no visit context.";
}
else $error = "Sorry, feature not available.";
...
if($_POST['action'] == 'send') {
	...
	else $msg = 'Message Sent';
	
	//echo "BANG! ".globalURL("sms-prov-client.php?appt=$apptid&msg=".urlencode($msg)."&error=".urlencode(join("<br>", $errors)));exit;
	globalRedirect("sms-prov-client.php?appt=$apptid&msg=".urlencode($msg)."&error=".urlencode(join("<br>", $errors)));
	exit;
}
// END MESSAGE SENDING
...
$showClientMessages = false; //loginidsOnlyTEST('dlifebeth');
/* TBD, if desired:
Add sitterCanSeeClientSMSStream after enableSitterToClientSMStd in mobile-communication-options-lightbox.php
 --est impl: sitterCanSeeClientSMSStream should depend on enableSitterToClientSMStd=true
Set showClientMessages = this pref
*/

if($showClientMessages) $messages = getSMSCommsFor($correspondent, $filter, $clientflg=true, $totalMsg=false);

$width = 500;
$textwidth = 80;;

require "frame-bannerless.php";
if($_GET['msg']) echo "<div class='pagenote fontSize1_5em bold'>{$_GET['msg']}</div>";
else if($_GET['error'])  echo "<div class='warning'>{$_GET['error']}</div>";
if($_GET['back']) 
...
$label = visitListClientLabel($appt);
echo "<h2>$backButton Mobile Message $label</h2>";
...
<p>
<?
$recipientType = 'client'; // $_GET['client'] ? 'client' : ($_GET['provider'] ? 'provider' : ($_GET['user'] ? 'user' : ''));
...
<textarea rows=3 cols=60 id='msg' name='msg' maxlength=160 onkeyup='countDownChars(this, 160)' style='font-size:2.0em'><?= $msgToSend ?></textarea>
</form>
<?
}
...
if($showClientMessages) echo $refreshButton;
