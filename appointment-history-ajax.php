...
else $appt = getAppointment($id, $withNames=true, $withPayableData=false, $withBillableData=false);
$dates = array();
...
$lognotes = fetchRows("SELECT time, note, user FROM tblchangelog WHERE itemptr = $id AND itemtable = 'tblappointment'");

foreach($lognotes as $note) 
	$dates[] = array($note[0], $note[1].' by '.getUserNameWithTip($note[2]));
	
$tracks = fetchAssociations("SELECT date, event, userptr FROM tblgeotrack WHERE appointmentptr = $id AND event != 'mv'");
foreach($tracks as $track) {
	$event = ($track['event'] == 'completed' ? 'Completion' : 'Arrival')." tracked by ".getUserNameWithTip($track['userptr']);
	$dates[] = array($track['date'], $event);
}
