<?
// service-types-standard.php  -- NOT HOURLY
...
$oldServiceTypes = getAllServiceNamesById();
...
if($_POST) {
  ...
  for($i=1; $i <= $lastvisible; $i++) {
		if($deletions && in_array("delete_$i", $deletions)) {
			...
			$service['extrapetcharge'] = $_POST["extrapetcharge_$i"];
			$rate = $_POST["defaultrate_$i"];
			if(strpos($rate,'%')) $rate = substr($rate, strpos($rate,'%'));
			$service['defaultrate'] = $rate;
			$rate = $_POST["extrapetrate_$i"];
			if(strpos($rate,'%')) $rate = substr($rate, strpos($rate,'%'));
			$service['extrapetrate'] = $rate;
			$service['ispercentage'] = isset($_POST["ispercentage_$i"]) && $_POST["ispercentage_$i"] ? 1 : 0;
...
//if(mattOnlyTEST()) {echo "<hr>Inserting #$i: ".print_r($service,1)."<hr>".print_r($_POST,1);}
	...
	if(!$remainhere) {  // this may be obselete
...
function hoursCell($i, $val, $exclusive) {
	if($exclusive) {
		$boldIfExclusive = 'font-weight:bold;';
		$title = "title='Exclusive service'";
	}
	$s = "<span id='hoursdisplay_$i' onclick='editHours(\"$i\")' style='text-decoration:underline;$boldIfExclusive' $title>$val</span>";
...
$columns = $columns = explodePairsLine("delete|Delete||label|Service||taxable|Taxable||defaultcharge|Price||extrapetcharge|Add'l Pet Price ($currencyMark)||defaultrate|Pay Rate||extrapetrate|Add'l Pet Rate||ispercentage|% ?||active|Active||hours|Hours||descr|Description");
$oldTypeNames = array();;
$n = 0;
$data = array();
foreach($types as $key => $service) {
	...
	if(staffOnlyTEST() || dbTEST('valleypetsitting,doggiewalkerdotcom,doggywalkerarlington,doggywalkerwam')) {
	...
	$data["old_$key"]['extrapetcharge'] = "<input class='dollarcell' id='extrapetcharge_$n' name='extrapetcharge_$n' value='{$service['extrapetcharge']}' autocomplete='off'>";
	$data["old_$key"]['defaultrate'] = "<input class='dollarcell' id='defaultrate_$n' name='defaultrate_$n' value='{$service['defaultrate']}' autocomplete='off'>";
	$data["old_$key"]['extrapetrate'] = "<input class='dollarcell' id='extrapetrate_$n' name='extrapetrate_$n' value='{$service['extrapetrate']}' autocomplete='off'>";
	$checked = $service['ispercentage'] ? 'CHECKED' : '';
	$data["old_$key"]['ispercentage'] = "<input type='checkbox' id='ispercentage_$n' name='ispercentage_$n' $checked title='Is rate is a % of price?' onClick='highlightPercentages()'>";
...
for($n++; $n < $numtypes; $n++) {
	...
	$data["new_$n"]['defaultrate'] = "<input class='dollarcell' id='defaultrate_$n' name='defaultrate_$n' value='' autocomplete='off'>";
	$data["new_$n"]['extrapetrate'] = "<input class='dollarcell' id='extrapetrate_$n' name='extrapetrate_$n' value='' autocomplete='off'>";
	$data["new_$n"]['ispercentage'] = "<input type='checkbox' id='ispercentage_$n' name='ispercentage_$n' $checked title='Is rate is a % of price?' onClick='highlightPercentages()'>";
...
function lineProblems(i) {
  ...
  var useLabel = (label.length == 0) ? "Each service" : "["+label+"]";
	...
	var extrapetcharge = jstrim(document.getElementById('extrapetcharge_'+i).value);
	if(extrapetcharge && !isUnsignedFloat(extrapetcharge)) problems[problems.length] = useLabel+"'s additional pet price must be a number.";
	var rate = jstrim(document.getElementById('defaultrate_'+i).value);
	if(rate.length == 0) problems[problems.length] = useLabel+" must have a pay rate.";
	if(document.getElementById('ispercentage_'+i).checked) {
		if(!isUnsignedFloatOrPercent(rate) || parseFloat(rate) > 100)
			problems[problems.length] = useLabel+" rate must be a percentage less than or equal to 100%.";
	}
	// else make sure rate is a float
	else if(!isUnsignedFloat(price)) problems[problems.length] = useLabel+"'s rate must be a number.";
	
	var extrapetrate = jstrim(document.getElementById('extrapetrate_'+i).value);
	if(extrapetrate && !isUnsignedFloat(extrapetrate)) problems[problems.length] = useLabel+"'s additional pet rate must be a number.";
...
function highlightPercentages() {
	var box;
	for(var i=1; box = document.getElementById('ispercentage_'+i); i++) {
		var color = box.checked ? 'mistyrose' : 'white';
		box.parentNode.style.backgroundColor = color;
		document.getElementById('defaultrate_'+i).style.backgroundColor = color;
		document.getElementById('extrapetrate_'+i).style.backgroundColor = color;
	}
}
...
function updateHours(i) {
	...
	if(!hours.match(patt1)) {
		alert('Hours must be expressed in HH:MM format. Allowed range = 0:00 - 24:59');
		return;
	}
