...
/* for video prep puposes only */ //if(mattOnlyTEST()) $editButton = echoButton('', 'Edit Schedule', "document.location.href=\"$page?packageid={$schedule['packageid']}\"", null, null, 'noEcho');
...
//if(mattOnlyTEST()) screenLog("schedules: ".print_r($schedules,1));
...
function getCurrentNRPackage($packageid, $clientptr=null) {
	...
	if(!$thisPackage['current']) {
		$packageid = findCurrentPackageVersion($packageid, $clientptr, false);
//if(mattOnlyTEST()) echo "Current packageid: ($packageid)<br>";	
...
//if(mattOnlyTEST()) echo "Current thisPackage: ($packageid)".print_r($thisPackage,1);	
	}
	return $thisPackage;
}
