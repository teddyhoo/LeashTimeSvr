...
$pageTitle = "Tax Liability Report by Client";
...
if(!$print && !$csv) {
	...
	// ***************************************************************************
?>
	<span>Generate a Tax Liability Report for all uncanceled appointments.</span><p>
...
<?
	...
	header("Content-Disposition: inline; filename=Tax-Liability-By-Client.csv ");
	dumpCSVRow('Tax Liability By Client');
	dumpCSVRow("Report generated: ".shortDateAndTime('now', 'mil'));
	dumpCSVRow("Period: $start - $end");
}
else {
	$windowTitle = 'Tax Liability Report by Client';
...
if($start && $end) {
	$figureTaxes = true;
	$clientTaxRates =  array();
//if(mattOnlyTEST()) echo "[[ $start, $end ]]";	
	revenuesAndCommissionsByClient($start, $end, $clientTaxRates);
	dropProjectionApptTable();
	dropProjectionSurchTable();
	
	$baseRate = $_SESSION['preferences']['taxRate'];
	
	foreach($clientTaxRates as $clientid => $rates) {
		foreach($rates as $code => $rate)
			if($rate && $rate != $baseRate)
				if($rate !== '0' || !$taxRatesByClient[$clientid])
					$taxRatesByClient[$clientid] = $rate;
					
		if(!$taxRatesByClient[$clientid] && $taxRatesByClient[$clientid] !== '0')
			$taxRatesByClient[$clientid] = $baseRate;
	}
	taxesByClientTable($start, $end, false, $csv);
}
if(!$print & !$csv){
