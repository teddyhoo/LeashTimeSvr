...
$repeatingPackageFields = explode(',', 'startdate,weeklyadjustment,suspenddate,cancellationdate,resumedate,'.
																	'cancellationreason,notes,previousversionptr,current,effectivedate,prepaid');
...
function packageDescriptionHTML($package, $clientName=null, $alsoExclude='', $exclude='packageid,clientptr,previousversionptr,preemptrecurringappts,current,onedaypackage,irregular,prepaid,weeklyadjustment,') {
	...
	$type = $package['monthly'] ? 'Fixed Price Monthly' : 
		($package['onedaypackage'] ? 'One Day' : (isset($package['enddate']) ? 'Short Term' : 'Weekly Recurring'));
	...
	return $descr.packageServiceDescription($package);
}
...
function recurringPackageSummary($packageid) {
...
$testing = mattOnlyTEST() ? 1 : '0';  
	echo <<<FUNC
var testingServiceJS = $testing;	
...
function updateServiceVals(element, number, prefix) {
	...
	if(service == 0) {
		...
		var rate;
		if(testingServiceJS) rate = NEWlookUpProviderServiceRate(service, provider, charge, pets, allpets)		
		else {
			rate = lookUpProviderServiceRate(service, provider, charge);
	...
	//if(debug) alert(service+','+provider+','+lookUpExtraPetProviderRate(service, provider, extrapetcharge));
...
function NEWlookUpProviderServiceRate(service, provider, charge, pets, allpets) {
	...
	charge = baseCharge + extraPetCharge;
	...
	if(rate == -999) {
		...
		else rate =standardRate[0] + extraPetCharge;
	}
	return rate;
}
...
//echo "<hr>CONFLICTS: ".print_r($conflicts, 1);  exit;
  ...
  notifyStaffOfScheduleChange($packageid, false);
	return $outData;
}
...
//$nonrepeatingPackageFields = explode(',', 'startdate,enddate,packageprice,departuredate,cancellationdate,returndate,cancellationreason,notes,howtocontact');
	...
	if(count(array_diff($changes, array('departuredate','returndate','cancellationdate', 'cancellationreason', 
																				'notes', 'howtocontact'.'preemptrecurringappts', 'previousversionptr', 'current', 'packageprice'))) == 0) {
		$outData['clientptr'] = $_POST['client'];
...
//exit;
  ...
  global $conflicts;
  $conflicts = findConflicts($_POST['client'], $newpackageid, $recurring=1);
...
function findCurrentPackageVersion($packageid, $clientptr, $recurring) {
...
function findPackageHistories($clientptr, $RorNorNull=null, $currentOnly=false) {
	...
	if(!$RorNorNull || $RorNorNull == 'R')
		foreach(fetchAssociationsKeyedBy(str_replace('XXX', 'tblrecurringpackage', $sql), 'packageid')
							as $key => $val)
				$packs[$key] = $val;
	if(!$RorNorNull || $RorNorNull == 'N')
		foreach(fetchAssociationsKeyedBy(str_replace('XXX', 'tblservicepackage', $sql), 'packageid')
							as $key => $val)
				$packs[$key] = $val;
...
function buildPackageHistory(&$pairs, $pack) {
	...
	else return array_merge(buildPackageHistory($pairs, $prev), array($pack));
}
	...
	$futureAppointmentRows = fetchAssociations(tzAdjustedSql(
	  "SELECT * FROM tblappointment
	    WHERE clientptr = $clientptr AND custom AND completed IS NULL AND canceled IS NULL $historyCheck
	      AND(date > CURDATE() OR (date = CURDATE() AND starttime > CURTIME()))"
	      .(!$endDate ? '' : "AND date >= '$startDate' AND date <= '$endDate'")
	      ));
  foreach($futureAppointmentRows as $row) 
		$customConflicts[$row['date']][] = $row;
	return $customConflicts;
...
*/	
	...
	if($provider) {
		if($provider > 0 && $explain) $name = fetchRow0Col0("SELECT CONCAT_WS(' ', fname, lname) FROM tblprovider WHERE providerid = $provider LIMIT 1");
		if($providerRates === null) $providerRates = getProviderRates($provider);
		if(isset($providerRates[$service])) { /* if CUSTOM RATE */
			$customRate = $providerRates[$service];
			
			if($customRate['ispercentage']) {
				if($explain) $explanation['rateApplied'] = 
					"Sitter's custom rate for $serviceName: {$customRate['rate']} X "
				...
				if($extraPetRatePercent > $customRate['rate']) {
					...
					if($explain) {
						$explanation['rateApplied'] = 
							"$name's custom rate for $serviceName: "
							.$customRate['rate'].' %';
						$explanation['customExtraPetRate'] = 
							"Custom Extra Pet Rate per pet: {$customRate['rate']}%  X ".dollarAmount($standardRate['extrapetcharge'])
						...
						$explanation['rateFinal'][] = "Rate = ".dollarAmount($customRate['rate'] / 100 * $baseCharge)
																						." + (".$customRate['rate']."% X "
																						.dollarAmount($standardRate['extrapetcharge']).")";
						$explanation['rateFinal'][] = "Rate = ".dollarAmount($rate);
					}
				...
				if($explain) $explanation['rateApplied'] = 
					"Sitter's custom rate for $serviceName: ".dollarAmount($customRate['rate']);
...
//echo "[[[".($baseCharge * $standardRate['defaultrate'] / 100)."]]]	+ 		$extraPetCharge";
			...
			$rate =$standardRate['defaultrate'] + $extraPetCharge;
			if($explain) $explanation['rateApplied'] = 
		...
		if($explain) $explanation['rateFinal'][] = "Rate = ".dollarAmount($rate - $extraPetRateDollars)." + ".dollarAmount($extraPetRateDollars);
...
function CORRECTcalculateServiceRate($provider, $service, $pets, $allPets, $totalCharge, $providerRates=null, $standardRates=null) {
	// totalCharge should include additional pet charge, if any
	// $provider is a providerid
	// $service is the service type code
	// $providerRates are the custom rates for provider
...
*/	
	...
	$extraPetRatePercent = $standardRate['ispercentage'] 
		? $standardRate['extrapetrate']
		: (!$standardRate['extrapetcharge'] ? 0 : $standardRate['extrapetrate'] / (float)$standardRate['extrapetcharge'] * 100);
	$numExtraPets = countAdditionalPets($pets, $allPets);
	$extraPetCharge = $numExtraPets * $standardRate['extrapetcharge'];
	$baseCharge = $totalCharge - $extraPetCharge;
	...
	if(!isset($rate)) { // either no sitter specified or no sitter-specific rate 
		if($standardRate['ispercentage']) 
			$rate = $baseCharge * $standardRate['defaultrate'] / 100 + $extraPetCharge * $extraPetRatePercent / 100;
		else $rate =$standardRate['defaultrate'] + $extraPetRateDollars * $numExtraPets;
	}
	return $rate;
}
...
function dumpRecurringSchedule($schedule=null, $clientid=null) {
	if(!$schedule) {
...
//echo "SELECT servicecode, charge, adjustment FROM tblservice WHERE packageptr = {$schedule['packageid']}<p>[[".print_r($services,1)."]]";	
	...
	if($schedule['packageid'] && $_SESSION['staffuser']) {
...
function dumpNonRecurringSchedules($clientid) {
	...
	if($schedules) {
		$allServiceNames = getAllServiceNamesById();
		$histories = findPackageHistories($client['clientid']);
		$nrstotshow = 10;
...
//if($schedule['onedaypackage']) {echo "[$serviceNames] ".print_r($services);exit;}
			...
			$serviceNames = nonRecurringCalendarLink($schedule['packageid'], $schedule)." <a href='$page?packageid={$schedule['packageid']}'>$serviceNames</a>";
			$start = shortDate(strtotime($schedule['startdate'])).
			...
			if(adequateRights('#ac')) {
				...
				$status .= "<!-- look in service-fns.php --><img width=16 height=10 src='art/email-invoice.gif' title='Email an invoice for these dates.' style='cursor:pointer;margin-left:5px;;margin-right:10px;' onclick='viewServicesInvoice(\"$day1\", \"$day2\", 1)'>";
			}
...
function fetchAllAppointmentsForNRPackage($packageidOrCurrentPackage, $clientptr=null) {
	$thisPackage = is_array($packageidOrCurrentPackage) 
									...
									: getCurrentNRPackage($packageid, $clientptr);
	if(!$clientptr) {
