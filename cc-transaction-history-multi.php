<? // cc-transaction-history-multi.php
...
$messages =  fetchAssociations($sql = 
	"SELECT * FROM tblchangelog 
	 WHERE 
	 	(itemtable IN ('ccpayment', 'ccrefund', 'ccpaymentadhoc', 'ccrefundadhoc') OR
	 	itemtable IN ('achpayment', 'achrefund'))
...
foreach($messages as $msg) {
	if(!$bydate && $msg['clientid'] != $lastClient) {
		...
		$row = array('#CUSTOM_ROW#'=>"<tr><td colspan=8 style='font-size:1.2em;font-weight:bold;'>{$msg['clientname']} [id: {$msg['clientid']}]</td></tr>");
		$rowClasses[] = '';
		$rows[] = $row;
	}
	$row = array();
...
// =================================================================
$pageTitle = "Electronic Transactions since $date";
...
function go(csv) {
	...
	document.location.href="cc-transaction-history-multi.php?bydate="+bydate+"&start="+start+"&csv="+csv;
}
<? dumpPopCalendarJS(); ?>
</script>
	<?
