...
if(userRole() == 'p') {
	$locked = locked('o-,d-'); // managers only
	...
	$locked = locked('o-');
	$editable = true;
	$pastEditingAllowed = FALSE;
}	
$editable = FALSE;

$blackoutsEnabled = FALSE; fetchRow0Col0("SELECT value FROM tblpreference WHERE property = 'enableTimeoffBlackouts'", 1); //dbTEST('dogslife');
$options = array('-- All Sitters --'=>0);
if(FALSE && $blackoutsEnabled) $options['BLACKOUT'] = getTimeOffBlackoutId(); // provider-fns.php
...
function allActiveProviderIdsAlphabetically() {
	return fetchKeyValuePairs("SELECT CONCAT(lname, ' ', fname), providerid FROM tblprovider WHERE active = 1 ORDER BY lname, fname", 1);
}

function allTimesAvailableThisDay($date, $provid, $editableTimeOff=false) {
	global $options, $blackoutsEnabled, $provs;
	
	$nullIfUnavailable = TRUE;
	...
	if($provid) { // ONE PROVIDER
			...
			foreach($options as $nm => $p) {
				if($p != $provid && $p != $timeOffBlackoutId && in_array($p, $slackers))
					$content .= 
						...
						.availabilityThisDayForOneProvider($date, $p, $editableTimeOff, 'omit', $nullIfUnavailable)
						."</span>";
			}
		}
	}
		...
		foreach($options as $nm => $p) {
			if($p != $timeOffBlackoutId && in_array($p, $slackers))
				$content .= availabilityThisDayForOneProvider($date, $p, $editableTimeOff, null, $nullIfUnavailable);
		}
	}
	return $content;
}

function availabilityThisDayForOneProvider($date, $provid, $editableTimeOff=false, $omitName=false, $nullIfUnavailable=false) {
	...
	foreach($times as $i => $t) {
		if(!$t['timeofday']) {
			...
			if($nullIfUnavailable) return null;
		}
		else {
			$briefly = explode('-', briefTimeOfDay($t));
			if(strpos($t['timeofday'], '12:00 am') === 0) $label = "after {$briefly[1]}";
			else $label = "before {$briefly[0]} and after {$briefly[1]}";
		}
...
function echoMonthBar($month, $day, $provid, $editableTimeOff=false) {
	...
	$baseLink = "availability-sitter-calendar.php?provid=$provid&editable=$editableTimeOff&date=";
...
$windowTitle = "Sitter Availability".($provid ? " for {$provs[$provid]}" : '');
...
if(userRole() != 'p') {
	...
	if(FALSE) // timeoffreportenabled enabled for all elsewhere 11/19/2020 
...
if($provid) {
	...
	sort($months);
	$baseLink = "availability-sitter-calendar.php?provid=$provid&editable=$editable&date=";
...
for($day = $start; $day <= $end; $day = date('Y-m-d', strtotime('+1 day', strtotime($day)))) {
	...
	if(!$dow) echo "</tr><tr>";
	echoDayBox($day, $provid, $editable=FALSE);
	$dayN++;
}
if($dow && $month) {  // finish prior month, if any
...
function refresh()
{
	...
	window.location.href='availability-sitter-calendar.php?provid='+sitter+'&editable=<?= $editable ?>&date=<?= $date ?>';
}
